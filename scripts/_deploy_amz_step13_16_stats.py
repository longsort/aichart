...
"""Deploy AMZ STEP13-16 outcome/replay/stats."""
...
FILES = [
    ...
    "lib/aiMarketZone/outcomeConfig.ts",

    "lib/aiMarketZone/outcomeEngine.ts",

    "lib/aiMarketZone/replayEngine.ts",

    "lib/aiMarketZone/statsTypes.ts",

    "lib/aiMarketZone/empiricRates.ts",
...
def main() -> int:
    ...
    try:
        ...
        run(ssh, f"grep -n 'computeAmzStatsFromCandles\\|runAmzReplay' {REMOTE}/lib/aiMarketZone/index.ts | head -4")
        ...
        run(
            ...
            "curl -sf 'http://127.0.0.1:3000/api/ai-market-zone-stats?symbol=BTCUSDT&timeframe=4h&rebuild=1' | head -c 400; echo",

        )

        out("[done] AMZ STEP13-16 OK")
