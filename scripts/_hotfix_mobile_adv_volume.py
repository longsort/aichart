#!/usr/bin/env python3
"""Hotfix: mobile advanced volume bars visible (volume panel height)."""
...
def main() -> int:
    ...
    try:
        ...
        run(
            ssh,
            f"grep -n 'volPanelTopRatio: 0.8' {REMOTE}/lib/volumeAiZoneEngine.ts | head -3",
        )
        ...
        out("[done] mobile adv-volume panel hotfix OK -> /root/ailongshort")
    finally:
        sftp.close()
        ssh.close()
    return 0
