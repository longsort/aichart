#!/usr/bin/env python3
"""어제~오늘 VWAP/MSB/zone/휠줌 패치 → /root/ailongshort + build + pm2."""
...
FILES = [
    ...
    "lib/vwap/index.ts",
    "lib/vwap/context.ts",
    "lib/vwap/paintGate.ts",
    "lib/vwap/sessionVwap.ts",
    "lib/vwap/avwapLineSignal.ts",
    "lib/vwap/avwapEntryCandidate.ts",
    "lib/vwap/avwapEntryGuide.ts",
    "lib/mergedDeskAnchoredVwap.ts",
    # Zoom + explain + ST zones
    "lib/chartSmoothWheelZoom.ts",
    "lib/chartFeatureExplain.ts",
    "lib/institutionalBandTouchZones.ts",
    "lib/mergedAnalysisDeskMarkers.ts",
...
def main() -> int:
    ...
    try:
        ...
        run(ssh, "sleep 3; pm2 list | sed -n '1,20p'")
        run(ssh, "curl -sf -o /dev/null -w 'home=%{http_code}\\n' http://127.0.0.1:3000/ || true")
        # smoke: key patches present
        run(
            ssh,
            "grep -c shouldPaintAvwapOnChart /root/ailongshort/lib/vwap/paintGate.ts; "
            "grep -c MERGED_DESK_ST_TOUCH_AS_ZONES /root/ailongshort/lib/institutionalBandTouchZones.ts; "
            "grep -c '구조돌파 MSB' /root/ailongshort/lib/mergedDeskRbSmcPois.ts; "
            "grep -c applySmoothCandleWheelZoom /root/ailongshort/lib/chartSmoothWheelZoom.ts",
        )
        out("[done] patch deploy OK -> /root/ailongshort")
    finally:
        sftp.close()
        ssh.close()
    return 0
