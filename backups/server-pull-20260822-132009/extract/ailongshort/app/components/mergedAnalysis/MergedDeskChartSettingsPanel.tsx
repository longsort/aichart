...
export function MergedDeskChartSettingsPanel({ open, onOpenChange, labelTargets }: Props) {
  ...
  useEffect(() => {
    ...
    const onHide = () => setHiddenTextTick((v) => v + 1);
    ...
    >
      <header className="merged-desk-chart-settings-panel__head">
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ fontWeight: 800, fontSize: 14, color: '#e0f2fe' }}>통합차트 스타일</div>
          <div style={{ fontSize: 10, color: '#94a3b8', marginTop: 1 }}>
            이 차트 분석만 · ON/OFF · 색 · 농도(연하게)
          </div>
        </div>
