#!/usr/bin/env python3
"""P3 MarketScalp/ZoneSniper/StructuralSL/NETEV/Flow deploy."""
...
FILES = [
    ...
    "lib/eagle1Tapoint/flowConfirmTap.ts",
    "lib/eagle1Tapoint/execKindEngine.ts",
    "lib/eagle1Tapoint/structuralSlTap.ts",
    "lib/eagle1Tapoint/tpCandidateEngine.ts",
    "lib/eagle1Tapoint/netEvFeeGate.ts",
]
...
def main():
    ...
    _, o, _ = ssh.exec_command(
        f"grep -n evaluateTapNetEvGate {remote}/lib/eagle1Tapoint/orchestrator.ts | head -3"
    )
    print(o.read().decode(), flush=True)
    ssh.close()
    print("P3 deploy OK", flush=True)
