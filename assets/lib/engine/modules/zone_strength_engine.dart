...
/// Active Zone(확정 구간)에 대해
/// - 실시간 체결/오더북으로 “방어(흡수)”와 “뚫림(돌파압력)” 점수
/// - 과거 유사(캔들 기반) % 통계
class ZoneStrengthEngine {
  static Future<ZoneStrength> analyze({
    ...
    // 1) 실시간 데이터
    final fills = await BitgetApi.getRecentFills(category: category, symbol: symbol, limit: 100);
  ...
  static int _absorptionScore(
    ...
    // imbalance는 지지에서는 +면 가점, 저항에서는 -면 가점
  ...
  static double _baselineVol(List<Candle> candles) {
    // 캔들 volume 평균을 베이스로 사용(데이터 없으면 1)
  ...
  static int _breakoutScore(
    ...
    // 초보용: “벽 얇음 + 한쪽 체결 우위 + 계속 눌러붙음”
    final last = candles.isNotEmpty ? candles.last : null;
    final totalVol = max(1e-9, buyVol + sellVol);
    final imbalance = ((buyVol - sellVol) / totalVol).clamp(-1.0, 1.0);

    // 오더북 벽: 지지면 bids, 저항면 asks, 박스면 둘 다 얕은 쪽
    final wall = _wallQty(zone, ob);
    final wallScore = (100 - min(100.0, wall * 10).round()).clamp(0, 100); // 벽이 클수록 돌파 어렵다

    int pressScore = 50;
    if (last != null) {
      // 접근 속도: 최근 5봉에서 zone 근처로 자주 오면 압력
    ...
    // imbalance 방향: 지지는 매도 우세면 하방 돌파 압력, 저항은 매수 우세면 상방 압력
    double imb = 0.0;
