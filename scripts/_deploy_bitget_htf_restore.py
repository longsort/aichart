#!/usr/bin/env python3
"""Deploy 1w/1M Bitget candle restore (match relax + true recent + full queue)."""
...
def main() -> int:
    ...
    try:
        ...
        run(
            ssh,
            f"grep -n 'enqueueFullHistory\\|5 \\* day\\|isHtf\\|htfBarStepMs' "
            f"{REMOTE}/lib/bitgetFuturesMarket.ts "
            f"{REMOTE}/lib/clientMarketCandleCache.ts "
            f"{REMOTE}/lib/volumeHistogramIntelligence.ts | head -n 30",
        )
        ...
        sftp.close()
        ssh.close()
    out("[done] 1w/1M Bitget restore deploy OK")
    return 0
