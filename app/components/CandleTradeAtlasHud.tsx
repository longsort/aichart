...
const LS_KEY = 'ailongshort-trade-atlas-hud-v2';
const LS_KEY_MERGED_RIGHT = 'ailongshort-trade-atlas-hud-merged-right-v2';

type Layout = { hidden: boolean; left: number | null; top: number | null; collapsed: boolean };

function loadLayout(storageKey: string, defaultHidden = false): Layout {
  try {
    const raw = typeof window !== 'undefined' ? window.localStorage.getItem(storageKey) : null;
    if (!raw) {
      return { hidden: defaultHidden, left: null, top: null, collapsed: false };
    }
...
export default function CandleTradeAtlasHud({
  ...
  /** 저장값 없을 때 패널 숨김(폰·통합·분석) */
  defaultHidden?: boolean;
  /** 숨김 → chartTradeAtlasShowHud OFF · Atlas HUD 칩과 동기 */
  onHideHud?: () => void;
}) {
  const storageKey = defaultAnchor === 'right' ? LS_KEY_MERGED_RIGHT : LS_KEY;
  const hudWidth = narrowUi ? 300 : 320;
  const [layout, setLayout] = useState<Layout>(() => loadLayout(storageKey, defaultHidden));
  ...
  const stopBtnPointer = (e: React.PointerEvent) => {
    e.stopPropagation();
  };
  ...
  if (layout.hidden) {
    return (
      <button
        type="button"
        className="tool-chip tool-chip-button trade-atlas-hud-restore"
        style={{
          position: 'absolute',
          left: 8,
          bottom: 8,
  ...
  return (
    ...
    >
      <div
        className="trade-atlas-hud__panel"
        style={{
          pointerEvents: 'auto',
          borderRadius: 14,
      ...
      >
        <div
          className="trade-atlas-hud__head"
          onPointerDown={(e) => {
        ...
        >
          ...
          <div className="trade-atlas-hud__actions" style={{ display: 'flex', gap: 4 }}>
            <button
              type="button"
              className="tool-chip tool-chip-button trade-atlas-hud__btn"
              style={{ padding: '6px 10px', fontSize: 10, minHeight: 36, touchAction: 'manipulation' }}
              onPointerDown={stopBtnPointer}
