...
import {
  ...
  isMergedDeskCandleBattleForecastOverlay,
...
function snapOverlayItemTimesToCandles(raw: OverlayItem, candles: Candle[]): OverlayItem {
  /** 이동경로·전투 예상경로·미래 투영은 마지막 봉 이후 시각 유지 */
  if (isMergedDeskWaveMovePathOverlay(raw) || isMergedDeskCandleBattleForecastOverlay(raw)) return raw;
  const hasTime =
