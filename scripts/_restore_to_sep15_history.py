# -*- coding: utf-8 -*-
"""Restore ailongshort workspace files from Cursor local history to 2026-09-15 (2 days ago)."""
...
# 2일 전: 2026-09-15 하루의 마지막 스냅샷
CUTOFF = datetime(2026, 9, 16, 0, 0, 0)
HIST = Path.home() / "AppData/Roaming/Cursor/User/History"
ROOT = Path(r"d:\apps\ailongshort")
BACKUP = ROOT / "docs" / "restore-backup-before-sep15-rollback"
...
def main() -> None:
    ...
    for d in HIST.iterdir():
        ...
        restored.append(f"{path} <- {best[1]} @ {best[0].isoformat(timespec='seconds')}")
