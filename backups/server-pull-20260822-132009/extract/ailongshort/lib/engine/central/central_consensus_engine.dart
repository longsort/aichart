...
import 'package:ailongshort/engine/central/decision_logger.dart';
...
class CentralConsensusEngine {
  ...
  void start() {
    ...
    // Heartbeat: keep UI "?∞Îèô" status moving even when external tick stalls.
  ...
  void _tick() {
    ...
    // Ï§ëÏïô ?îÏßÑ ??UI ?∞Îèô ?ÅÌÉú ?êÎã®??    ConsensusBus.I.lastUpdateMs.value = DateTime.now().millisecondsSinceEpoch;
  ...
  _EvidenceRes _calcEvidenceSafe(_CentralConsensusRes res) {
    try {
      ...
      final flags = <String, bool>{
        '?ÅÏúÑ(4H¬∑1D) ?ôÏ°∞': higherOk,
        '?§Ïúô(15m¬∑1H) ?ôÏ°∞': swingOk,
        'Ï¥àÎã®?Ä(1¬∑3¬∑5Î∂? ?Ä?¥Î∞ç': scalpOk,
        'Î™®Î©ò?Ä Ï∂©Î∂Ñ': momOk,
        'Í±∞Îûò??Ï¶ùÍ?': volUp,
        'Í≥†Îûò ?±Í∏â(Ï§ë‚Üë)': whaleOk,
        'Î≥Ä?ôÏÑ± ?àÏ†ï': volaOk,
        'Ï§ëÏïô Î∞©Ìñ• ?ºÏπò': alignOk,
        'Î¶¨Ïä§??OK': riskOk,
        '?∞Ïù¥???ïÏÉÅ(?∞Í≤∞)': true,
      };
