...
FILES = [
    "lib/eagle1Tapoint/htfSweepFilter.ts",
    ...
    "lib/mergedDeskCoinExclusiveSkills.ts",
    "lib/serverCoinExclusiveSkillsStore.ts",
    "app/api/merged-desk/coin-exclusive-skills/route.ts",
    "lib/mergedDeskSkillColors.ts",
    "lib/mergedDeskCoinSkillBoard.ts",
...
def main():
    ...
    run(
        f"grep -n 'evaluateHtfSweepFilter\\|htfSweepFilter\\|skillColors\\|coin-exclusive' "
        f"{remote}/lib/eagle1Tapoint/htfSweepFilter.ts "
        f"{remote}/lib/mergedDeskSkillColors.ts "
        f"{remote}/app/api/merged-desk/coin-exclusive-skills/route.ts "
        f"| sed -n '1,20p'"
    )
    print("htf+exclusive+colors OK", flush=True)
    ssh.close()
    return 0
