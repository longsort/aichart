...
/// 교체 카드: 롱숏 ?�률 · ?�심구간 · 지지반등 (메인/미래경로 공용)
/// CSV 카드 ?�???�용 ???�시�??�진 기�? 롱숏 결정 + 구간 ?�시
class LongShortZoneCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final zoneName = s.zoneName.isNotEmpty ? s.zoneName : '구간?�보?�음';
    ...
    if (decisionLong && longPct >= shortPct) {
      ...
      decisionText = '관�?L${longPct}% S${shortPct}%';
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          const SizedBox(height: 10),
          Text(
            '?�심구간(?��?): ${rLow > 0 && rHigh > 0 ? "${rLow.toStringAsFixed(0)} ~ ${rHigh.toStringAsFixed(0)}" : "-"}${breakLvl > 0 ? " · ?�파 ${breakLvl.toStringAsFixed(0)}" : ""}',
