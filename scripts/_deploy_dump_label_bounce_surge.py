#!/usr/bin/env python3
"""Deploy bounce/dump contextual labels: 폭등감시 vs 폭락감시."""
...
def main() -> int:
    ...
    try:
        ...
        run(
            ssh,
            f"grep -n 'dumpBandFaceRoleKo\\|폭등감시\\|반등지지' "
            f"{REMOTE}/lib/mergedDeskDumpLifeCycle.ts "
            f"{REMOTE}/lib/mergedDeskMtfDumpZoneBridge.ts | head -n 30",
        )
        ...
        sftp.close()
        ssh.close()
    out("[done] dump label bounce/surge deploy OK -> /root/ailongshort")
    return 0
