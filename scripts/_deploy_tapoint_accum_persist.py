...
FILES = [
    "lib/tapointAccumDirty.ts",
    "lib/tapointAccumServerPersist.ts",
    "lib/tapointAccumClient.ts",
    "app/api/eagle1/tapoint-accum/route.ts",
    "lib/mergedDeskSignalScorecard.ts",
    "lib/eagle1Tapoint/rejectLedger.ts",
    "lib/mergedDeskTradeEventJournal.ts",
    "lib/mergedDeskVirtualSeedLedger.ts",
    "lib/mergedDeskPositionEntryLabel.ts",
    "lib/mergedDeskCoinStatsServerPersist.ts",
...
def main():
    ...
    # 누적 디렉터리만 보장 · 절대 삭제하지 않음
    run(f"mkdir -p {remote}/data/tapoint-accum {remote}/data/merged-desk {remote}/data/auto-trade-arm")
    ...
    run(
        f"grep -n 'mergeTapointAccumBlob\\|hydrateTapointAccumFromServer\\|markTapointAccumDirty' "
        f"{remote}/lib/tapointAccumServerPersist.ts "
        f"{remote}/lib/tapointAccumClient.ts "
        f"{remote}/lib/mergedDeskSignalScorecard.ts "
        f"| sed -n '1,20p'"
    )
    print("tapoint accum persist OK", flush=True)
    ssh.close()
    return 0
