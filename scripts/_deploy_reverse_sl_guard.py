...
FILES = [
    "lib/mergedDeskDirectionSlGuard.ts",
    "lib/mergedDeskRbScalpDriveTrade.ts",
    "lib/mergedDeskDualStructureEntry.ts",
...
def main():
    ...
    run(
        f"grep -n '역방향손절\\|assertRailSide\\|DIR_SL_TP_REJECT\\|direction:' "
        f"{remote}/lib/mergedDeskDirectionSlGuard.ts "
        f"{remote}/lib/mergedDeskRbScalpDriveTrade.ts | head -20"
    )
    print("\nOK reverse-sl-guard deploy", flush=True)
    ssh.close()
    return 0
