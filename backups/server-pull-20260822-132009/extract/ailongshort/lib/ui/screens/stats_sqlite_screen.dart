...
void initState() {
  super.initState();
  // 5Ï¥àÎßà???êÎèô ?àÎ°úÍ≥†Ïπ® + ?†Ìò∏/Í≤∞Í≥º Í∏∞Î°ù ??Ï¶âÏãú ?àÎ°úÍ≥†Ïπ®
...
void dispose() {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: const Text('?µÍ≥Ñ(?êÏú®Î≥¥Ï†ï)', style: TextStyle(fontWeight: FontWeight.w900)),
      ),
            ...
            return Center(
              child: Padding(
                ...
                child: Column(
                  ...
                  children: [
                    const Text('?µÍ≥Ñ Î°úÎî© ?§Ìå®', style: TextStyle(color: Colors.white, fontWeight: FontWeight.w900)),
                    const SizedBox(height: 8),
                    Text('${snap.error}', style: const TextStyle(color: Colors.white70, fontSize: 12)),
                    const SizedBox(height: 12),
                    ElevatedButton(onPressed: () => setState(() {}), child: const Text('?¨Ïãú??)),
                  ],
                ),
              ),
            );
          ...
          return RefreshIndicator(
            ...
            child: ListView(
              ...
              children: [
                      ...
                      children: [
                        Row(
                          children: [
                            ...
                            const Text('?îÏßÑ ?ÄÍ∏?Ï§?¬∑ ?ïÏ†ï ?†Ìò∏ ?Ä?????êÎèô Î∞òÏòÅ', style: TextStyle(color: Colors.white54, fontSize: 11, fontWeight: FontWeight.w800)),
                          ],
                        ),
                        const SizedBox(height: 8),
                        const Text(
                          '?ÑÏßÅ ?ïÏ†ï ?†Ìò∏/Í≤∞Í≥º Í∏∞Î°ù???ÜÏñ¥??0%Î°?Î≥¥ÏûÖ?àÎã§.\n(?†Ìò∏Í∞Ä ?Ä?•ÎêòÎ©??êÎèô?ºÎ°ú ?ÑÏ†Å/?êÏú®Î≥¥Ï†ï ?©Îãà??)',
                        ...
                        Row(
                          children: [
                            ElevatedButton(
                              ...
                              child: Text(_seeding ? '?ùÏÑ± Ï§ë‚Ä? : '?åÏä§???∞Ïù¥???ùÏÑ±'),
                            ),
                            const SizedBox(width: 10),
                            const Expanded(
                              child: Text(
                                '???§Ï†ú Îß§Îß§???ÑÎãò. DB¬∑?îÎ©¥ ?∞Îèô ?ïÏù∏??',
                ...
                if (d.rows.isNotEmpty) _card(title: '?îÏßÑ ?∞Îèô ?ÅÌÉú', child: _engineGauge(d)),
                _card(
                  title: '?ÑÏû¨ ?úÎãù ?åÎùºÎØ∏ÌÑ∞',
                  child: Wrap(
                    spacing: 10,
                    runSpacing: 8,
                    children: [
                      _pill('?ïÏ†ïÍ∏∞Ï? ${(d.params.thrConfirm * 100).toStringAsFixed(0)}%'),
                      _pill('ÏßÄÏßÄÍ∞ÄÏ§ëÏπò ${(d.params.wSupport * 100).toStringAsFixed(0)}'),
                      _pill('?Ä???Ï§ëÏπò ${(d.params.wResist * 100).toStringAsFixed(0)}'),
                      _pill('Íµ¨Ï°∞Í∞ÄÏ§ëÏπò ${(d.params.wStructure * 100).toStringAsFixed(0)}'),
                    ],
                  ),
                ),
                const SizedBox(height: 10),
                _card(
                  title: 'ÏµúÍ∑º 30(Í≤∞Í≥º ?àÎäî Í≤ÉÎßå)',
                  child: Wrap(
                    spacing: 10,
                    runSpacing: 8,
                    children: [
                      _pill('?πÎ•† ${d.winrate.toStringAsFixed(1)}%'),
                      _pill('?ÑÏ†Å?êÏùµ ${d.pnlSum.toStringAsFixed(2)}R'),
                      _pill('????${d.w}/${d.l}'),
                    ],
                  ),
                ),
                const SizedBox(height: 10),
                _card(title: '?úÍ∞Ñ?ÄÎ≥??πÎ•†(ÏµúÍ∑º 200)', child: _sessionTable(d.sessionRows)),
                const SizedBox(height: 10),
                _card(title: 'ÏµúÍ∑º ?†Ìò∏ 30', child: Column(children: d.rows.map(_row).toList())),
                const SizedBox(height: 10),
                _card(
                  title: '?úÎãù Î≥ÄÍ≤?Î°úÍ∑∏',
                  child: Column(
  ...
  Widget _sessionTable(List<Map<String, Object?>> rows) {
    if (rows.isEmpty) return const Text('?∞Ïù¥???ÜÏùå', style: TextStyle(color: Colors.white70, fontWeight: FontWeight.w800));
    return Column(
      ...
      child: Row(
        children: [
          SizedBox(
            ...
            child: Column(
              ...
              children: [
                ...
                Text('?ïÏã† $conf%', style: const TextStyle(color: Colors.white70, fontSize: 11, fontWeight: FontWeight.w800)),
              ],
            ),
          ),
          Expanded(
          ...
          Container(
            ...
            child: Text(badge == 'WIN' ? '?? : (badge == 'LOSS' ? '?? : '?ÄÍ∏?), style: const TextStyle(color: Colors.white, fontSize: 11, fontWeight: FontWeight.w900)),
          ),
        ],
      ),
    );
      ...
      children: [
        ...
        Expanded(
          child: Column(
            crossAxisAlignment: CrossAxisAlignment.start,
            children: [
              const Text('?îÏßÑ ?∞Îèô??¬∑ ?†Ìò∏ ?∞Ïù¥??Î∞òÏòÅ Ï§?, style: TextStyle(color: Color(0xFF1EEA6A), fontSize: 12, fontWeight: FontWeight.w900)),
              ...
              const SizedBox(height: 4),
              Text('?πÎ•† Í≤åÏù¥ÏßÄ ${(wr * 100).toStringAsFixed(1)}% (??${d.w} / ??${d.l})', style: const TextStyle(color: Colors.white70, fontSize: 11, fontWeight: FontWeight.w800)),
            ],
          ),
        ),
      ],
