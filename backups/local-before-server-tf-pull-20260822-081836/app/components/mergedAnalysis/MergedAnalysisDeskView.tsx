...
export default function MergedAnalysisDeskView({
  ...
  const syncFullscreen = useCallback(() => {
    ...
    setChartFullscreen(document.fullscreenElement === el || cssOn);
  }, []);
  ...
  useEffect(() => {
    ...
    const onKey = (e: KeyboardEvent) => {
      ...
      if (el?.classList.contains('is-css-chart-fullscreen')) {
  ...
  const toggleChartFullscreen = async () => {
    ...
    /** 네이티브 FS는 트리 리마운트와 충돌 — CSS 고정 FS만 사용 */
  ...
  const onMergedDeskChartCandlesChange = useCallback((next: Candle[], chartTf?: string) => {
    ...
    /** TF 전환 중 빈 배열로 덮지 않음 — 단, 다른 TF 잔존 캔들은 절대 유지하지 않음 */
    if (next.length < 12) {
  ...
  /** 심볼·TF 변경 시 이전 TF 캔들 폐기 — 4h 존이 15m/5m에 남는 사고 방지 */
  useEffect(() => {
    setChartMarketPack({ candles: [], tf: timeframe });
  }, [symbol, timeframe]);

  /** 통합·분석·독수리1호 진입·심볼·TF 변경 시 전 TF 즉시 병렬 워밍 */
  useEffect(() => {
    if (uiMode !== 'MERGED_ANALYSIS_DESK' && uiMode !== 'EAGLE1_DESK') return;
    const s = loadSettings();
  ...
  /** TF 전환 직후: 이전 TF deskPack 시그니처 무효화 */
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    /** TF 전환 체감: idle/이중 rAF 제거 — 즉시 엔진(기능 유지) */
