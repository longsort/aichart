...
import type { CombinationMiningReport } from './combinationMiningEngine';
import type { ScoreCalibrationReport } from './scoreCalibrationView';
import type { PositionSizeReport } from './positionSizeEngine';
import type { HtfHistoricalStatus } from './htfHistoricalStatus';
import type { SmartFuturePathReport } from './smartFuturePathEngine';
import type { OrderFlowReport } from './orderFlowFacade';
import type { PositionManagementReport } from './positionManagementEngine';
import type { CandleEvidenceReport } from './candleEvidenceEngine';
import type { ProfileLevelsReport } from './profileLevelsEngine';
import type { CostAwareWalkForwardReport } from './costAwareWalkForward';
import type { LiquidityDefenseReport } from './liquidityDefenseEngine';
import type { HeatUnderlayPolicy } from './heatUnderlayPolicy';
import type { OverlayBudgetReport } from './overlayBudget';
import type { VisualSelftestResult } from './visualLayoutContract';
...
export type Eagle1HudPack = {
  ...
  executionLevels: ExecutionLevelsReport | null;
  combinationMining: CombinationMiningReport | null;
  scoreCalibration: ScoreCalibrationReport | null;
  positionSize: PositionSizeReport | null;
  htfHistorical: HtfHistoricalStatus | null;
  smartFuturePath: SmartFuturePathReport | null;
  orderFlow: OrderFlowReport | null;
  positionManagement: PositionManagementReport | null;
  candleEvidence: CandleEvidenceReport | null;
  profileLevels: ProfileLevelsReport | null;
  costAwareWalkForward: CostAwareWalkForwardReport | null;
  liquidityDefense: LiquidityDefenseReport | null;
  heatUnderlay: { policy: HeatUnderlayPolicy; bars: number; note: string } | null;
  overlayBudget: OverlayBudgetReport | null;
  visualLayout: VisualSelftestResult | null;
...
function entryZoneState(
  ...
  if (
    ...
    trade?.state === 'STOPPED_LOSS' ||
    trade?.state === 'STOPPED_PROFIT' ||
    trade?.state === 'EARLY_EXIT' ||
    trade?.state === 'CLOSED'
  ) {
    return 'INVALID';
  }
...
function tradeStep(plan: Eagle1MainPlan | null, trade: FrozenTrade | null): { current: HudTradeStep; note: string } {
  if (trade?.state === 'STOPPED_PROFIT') return { current: 'EXIT', note: '수익권 종료' };
  if (trade?.state === 'STOPPED_LOSS' || trade?.state === 'INVALIDATED') return { current: 'EXIT', note: '손절·무효' };
  if (trade?.state === 'EARLY_EXIT' || trade?.state === 'CLOSED') return { current: 'EXIT', note: '종료' };
  if (trade?.state === 'TP3_HIT') return { current: 'TP3', note: '목표3 도달' };
  if (trade?.state === 'TRAILING' || trade?.state === 'TP2_HIT') return { current: 'TRAIL', note: '목표2 이후 추적' };
  if (trade?.state === 'BREAKEVEN' || trade?.state === 'PROTECT_PROFIT') return { current: 'BE', note: '수익보호' };
...
export function buildEagle1HudPack(params: {
  ...
  const squeezeRadar = params.squeezeRadar ?? null;
  const bigMove =
    squeezeRadar?.bigMove ??
    runBigMoveEngine({
  ...
  const vd =
    params.live?.has_cvd && typeof params.live.volumeDelta === 'number'
      ? params.live.volumeDelta
      : params.money != null && Number.isFinite(params.money.buyShare)
        ? Math.round((params.money.buyShare - 0.5) * 200) / 100
        : null;
  ...
  return {
    ...
    volumeFlow: {
      ...
      labelKo:
        vd == null
          ? params.money?.stateKo ?? '데이터 없음'
          : params.live?.has_cvd
            ? vd >= 0
              ? '순매수 유입'
              : '순매도 유입'
            : params.money?.stateKo ?? (vd >= 0 ? '매수 우세(근사)' : '매도 우세(근사)'),
    },
    flowDirection: {
      labelKo:
        flowArrows === 0
    ...
    squeezeRadar,
    mtfSmartZone: params.mtfSmartZone ?? null,
    liqZones: params.liqZones ?? null,
    reEntry: params.reEntry ?? null,
    legendaryFusion: params.legendaryFusion ?? null,
    tradeOpportunity: params.tradeOpportunity ?? null,
    executionLevels: params.executionLevels ?? null,
    combinationMining: params.combinationMining ?? null,
    scoreCalibration: params.scoreCalibration ?? null,
    positionSize: params.positionSize ?? null,
    htfHistorical: params.htfHistorical ?? null,
    smartFuturePath: params.smartFuturePath ?? null,
    orderFlow: params.orderFlow ?? null,
    positionManagement: params.positionManagement ?? null,
    candleEvidence: params.candleEvidence ?? null,
    profileLevels: params.profileLevels ?? null,
    costAwareWalkForward: params.costAwareWalkForward ?? null,
    liquidityDefense: params.liquidityDefense ?? null,
    heatUnderlay: params.heatUnderlay ?? null,
    overlayBudget: params.overlayBudget ?? null,
    visualLayout: params.visualLayout ?? null,
