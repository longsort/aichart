#!/usr/bin/env python3
"""Deploy HTF candle identity fix: 1d/1w/1M distinct, no cross-TF residue."""
...
def main() -> int:
    ...
    try:
        ...
        run(
            ssh,
            f"grep -n 'candlesMatchChartTimeframe\\|chartTfStepSec' "
            f"{REMOTE}/lib/clientMarketCandleCache.ts "
            f"{REMOTE}/app/components/ChartView.tsx | head -n 20",
        )
        ...
        sftp.close()
        ssh.close()
    out("[done] HTF candle identity deploy OK")
    return 0
