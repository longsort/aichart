...
def main():
    ...
    run(f"mkdir -p {remote}/data/merged-desk-auto-trade-locks")
    ...
    run(
        f"grep -n 'USER_TICK_CONCURRENCY\\|runTapointDecideShared\\|recentCandleInflight' "
        f"{remote}/lib/mergedDeskServerAutoTradeRunner.ts "
        f"{remote}/lib/bitgetFuturesMarket.ts "
        f"{remote}/lib/serverTapointDecideShare.ts "
        f"| sed -n '1,25p'"
    )
    print("multi-user isolation OK", flush=True)
    ssh.close()
    return 0
