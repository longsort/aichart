...
import 'package:ailongshort/data/snapshot/snapshot_reader.dart';
import 'package:ailongshort/engine/stats/stats_calc.dart';
import 'package:ailongshort/data/bitget/bitget_live_store.dart';
import 'package:ailongshort/engine/guard/no_trade_guard_store.dart';
import 'package:ailongshort/engine/learning/evidence_weight_store.dart';
import 'package:ailongshort/engine/learning/evidence_learner.dart';
import 'package:ailongshort/ui/widgets/evidence_heatmap_card.dart';
import 'package:ailongshort/ui/widgets/heatmap_delta_card.dart';
import 'package:ailongshort/ui/widgets/learning_intensity_card.dart';
import 'package:ailongshort/ui/widgets/performance_last20_card.dart';
...
Outcome evalOutcomeAdvanced(Map<String, dynamic> s, double now, double minP, double maxP) {
  ...
  if (d != '?ÅÏäπ(LONG)' && d != '?òÎùΩ(SHORT)') return Outcome.open;
              ...
              _card(
                Column(
                  ...
                  children: [
                    ...
                    _row('?ÅÏäπ(LONG) rate', '${(sum.longRate * 100).toStringAsFixed(1)}%'),
                    _row('?òÎùΩ(SHORT) rate', '${(sum.shortRate * 100).toStringAsFixed(1)}%'),
                    ...
                    const Text('??v85-86: Ï¶ùÍ±∞Î≥?Í∞ÄÏ§ëÏπò ?ôÏäµ + ?àÌä∏Îß?+ ?†Î¢∞?±Í∏â.',
    ...
    return Container(
      ...
      child: Row(
        children: [
          ...
          Expanded(
            child: Text('Í¥ÄÎß?$txt  (lossStreak ${s.lossStreak})',
                style: const TextStyle(color: Colors.white, fontSize: 12, fontWeight: FontWeight.bold)),
          ),
        ],
      ),
...
// --- FIX: _paperBoxÍ∞Ä ?ÜÏùÑ ?åÎ? ?ÄÎπÑÌïú ?ïÏû• Î©îÏÑú????†ú ?ÜÏù¥ Ï∂îÍ?) ---
extension PaperBoxExt on _StatsScreenV82State {
  Widget _paperBox() {
    return ValueListenableBuilder(
      ...
      builder: (_, s, __) {
        final last = s.last;
        final win = last.where((e) => e.outcome == '?±Í≥µ').length;
        final loss = last.where((e) => e.outcome == '?§Ìå®').length;
        final tout = last.where((e) => e.outcome == '?úÍ∞ÑÏ¥àÍ≥º').length;
        ...
        return Container(
          ...
          child: Column(
            crossAxisAlignment: CrossAxisAlignment.start,
            children: [
              const Text('Í∞Ä??Îß§Îß§ Í≤∞Í≥º(???êÎèô)',
                  style: TextStyle(color: Colors.white, fontSize: 13, fontWeight: FontWeight.bold)),
              const SizedBox(height: 6),
              Text('?±Í≥µ $win / ?§Ìå® $loss / ?úÍ∞ÑÏ¥àÍ≥º $tout',
                  style: const TextStyle(color: Colors.white70, fontSize: 12, fontWeight: FontWeight.bold)),
              const SizedBox(height: 10),
              Row(
                children: [
                  const Text('?úÍ∞ÑÏ¥àÍ≥º Ï¢ÖÎ£å', style: TextStyle(color: Colors.white70, fontSize: 11)),
                  const SizedBox(width: 8),
                  ValueListenableBuilder<int>(
                    valueListenable: PaperTradeEngine.I.timeoutMinutes,
                    builder: (_, v, __) => Text('$vÎ∂?,
              ...
              const SizedBox(height: 8),
              if (last.isNotEmpty) ...[
                const Text('ÏµúÍ∑º 5Í∞?, style: TextStyle(color: Colors.white54, fontSize: 11)),
                const SizedBox(height: 6),
                ...last.take(5).map((e) => Text(
                      '${e.outcome} ???êÏùµ ${e.pnlUsd.toStringAsFixed(2)} USDT',
