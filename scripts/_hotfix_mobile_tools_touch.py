#!/usr/bin/env python3
"""Hotfix: mobile tools chips clickable (in-flow scroll + smaller chips)."""
...
def main() -> int:
    ...
    try:
        ...
        run(
            ssh,
            f"grep -n '인플로우 + 스크롤' {REMOTE}/app/components/mergedAnalysis/MergedAnalysisDesk.module.css | head -3",
        )
        ...
        out("[done] mobile tools touch hotfix OK -> /root/ailongshort")
    finally:
        sftp.close()
        ssh.close()
    return 0
