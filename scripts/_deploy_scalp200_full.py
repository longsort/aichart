#!/usr/bin/env python3
"""200x 타점 완성 패치 → /root/ailongshort + build + pm2."""
...
def main() -> int:
    ...
    try:
        ...
        run(
            ssh,
            f"test -f {REMOTE}/lib/mergedDeskScalp200Plan.ts && echo '[ok] scalp200 module on server'",
        )
        out("[done] 200x scalp200 full deploy OK -> /root/ailongshort")
    finally:
        sftp.close()
        ssh.close()
    return 0
