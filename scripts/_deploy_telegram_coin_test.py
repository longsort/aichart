...
FILES = [
    "app/api/merged-desk/telegram-test/route.ts",
...
def main():
    ...
    # env presence check (masked) — no secrets printed
    run(
        f"cd {remote} && node -e \"const t=!!(process.env.TELEGRAM_BOT_TOKEN||'').trim();"
        f"const c=!!(process.env.TELEGRAM_CHAT_ID||'').trim();"
        f"console.log('tg_env', JSON.stringify({{bot:t,chat:c}}))\""
    )
    run(
        f"grep -n 'telegram-test\\|runTelegramCoinTest\\|tg-test' "
        f"{remote}/app/api/merged-desk/telegram-test/route.ts "
        f"{remote}/app/components/mergedAnalysis/MergedDeskAutoTradePanel.tsx "
        f"| sed -n '1,20p'"
    )
    print("telegram coin test chips OK", flush=True)
    ssh.close()
    return 0
