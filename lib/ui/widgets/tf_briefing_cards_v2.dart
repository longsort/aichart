...
class TfBriefingCardsV2 extends StatelessWidget {
  ...
  String _titleFor(FuState s) {
    ...
    if (s.score >= 60) return 'ë¡??°ì„¸';
    if (s.score <= 40) return '???°ì„¸';
    return 'ê´€ë§?;
  }
  ...
  String _badgeFor(FuState? s) {
    ...
    if (t.contains('ë¡?) || t.contains('ë§¤ìˆ˜') || t.contains('?ìŠ¹')) return 'B';
    if (t.contains('??) || t.contains('ë§¤ë„') || t.contains('?˜ë½')) return 'S';
