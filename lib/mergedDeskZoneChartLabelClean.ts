...
export function compactMergedDeskZoneChartLabel(raw: string): string {
  ...
  /** 폭락·반등 밴드 — 롱/숏 접두 + 기능 상태 */
  if (/폭락|폭등|반등지지|반등구간|하락확정|상승확정|하락연동|상승연동/.test(s)) {
    const m = s.match(
      /^(?:(롱|숏)\s*)?(폭락구간|반등구간|폭락감시|폭등감시|반등지지|하락확정|상승확정|저항확정|\S+\s*(?:폭락감시|폭등감시|반등지지|폭락|하락확정|상승확정))\s*(하락확정|상승확정|나락확정|반등확정|저항확정|반등감시|저항감시|하락확정연동|하락연동|상승확정연동|상승연동|대기|감시|연동)?(?:\s*[·･]?\s*(\d{4,7}))?/
    );
    if (m) {
      return [m[1], m[2], m[3], m[4]].filter(Boolean).join(' ').slice(0, 22);
    }
    return s.replace(/\s+/g, ' ').slice(0, 22);
  }
  ...
  /** 최강분석zone — 12자 캡에 맞게 유지 */
  if (/★\s*최강/.test(s)) {
    const m = s.match(/★\s*최강[·･]?\s*(초강력|초강|강|중|약)?\s*(반등|하락|저항)?/);
    if (m) {
      const g = m[1] || '';
      const d = m[2] || '';
      return `★최강${g || d ? `·${g}${d}` : ''}`.slice(0, 12);
    }
    return '★최강분석';
  }
...
function captionPriority(o: OverlayItem): number {
  ...
  if (extra.includes('merged-desk-strongest-analysis-zone') || /★\s*최강/.test(text)) return 130;
  if (id.includes('mtf-dump') || extra.includes('merged-desk-mtf-dump-zone') || /폭락/.test(text))
    return 125;
...
export function applyMergedDeskZoneChartLabelClean(overlays: OverlayItem[]): OverlayItem[] {
  ...
  let next = overlays.map((raw, i) => {
    ...
    const extra = String(raw.overlayZoneExtraClass || '');
    /** 폭락구간 — 라벨·상태(감시→확정) 유지, 압축으로 상태 잘림 금지 */
    if (id.includes('mtf-dump') || extra.includes('merged-desk-mtf-dump-zone') || /폭락/.test(full)) {
      const face =
        String(raw.zoneFaceBase || '').trim() ||
        compactMergedDeskZoneChartLabel(full) ||
        full ||
        '폭락구간';
      ...
      return {
        ...
        label: face,
        zoneFaceBase: face,
        zoneFaceSignal: signal || undefined,
