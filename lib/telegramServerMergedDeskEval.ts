...
/** 중투·스윙·폭락·기관밴드 텔레 — 15m·1h·4h·1d·1w·1M */
export const MERGED_DESK_AUTO_ALERT_TFS = MERGED_DESK_SHARED_TELEGRAM_TFS;
...
export function buildTelegramMergedDeskEval(
  ...
  const sourceKo = pack.superStatsHub?.stats
    ? `AI超级变身统计 · ${pack.superStatsHub.stats.verdictKo} · 합의${pack.superStatsHub.agreementPct}%${
        pack.activeTradePlan?.entryAllowed ? ' · 진입허용' : ' · 진입보류'
      }`
    : at
