...
/// 타이롱 PRO v2 (개편):
/// - 꼬리 흡수(engulf/absorb) + RSI 모멘텀 + 볼륨 스파이크(간이) 기반
/// - 결과: bias/conf + "미래 경로(점선)"용 상대 수익률 시퀀스
///
/// NOTE: 본 엔진은 '결정력.zip' 프로젝트에서 안전하게 동작하도록
///       외부 의존성 없이(라이브러리 없이) 구현됨.
class TyronProEngine {
  static TyronProResult analyze(List<Candle> candles, {int rsiLen = 14, int pathLen = 18}) {
    ...
    if (bullAbsorb) {
      score += 0.55;
      reasons.add('아래꼬리 흡수(롱)');
    }
    if (bearAbsorb) {
      score -= 0.55;
      reasons.add('윗꼬리 흡수(숏)');
    }

    if (rsiSlope.abs() > 0.8) {
      score += rsiSlope > 0 ? 0.12 : -0.12;
      reasons.add(rsiSlope > 0 ? 'RSI 모멘텀↑' : 'RSI 모멘텀↓');
    }

    if (volSpike >= 1.35) {
      score += score >= 0 ? 0.10 : -0.10;
      reasons.add('거래량 스파이크');
    }
    ...
    if (reasons.isEmpty) reasons.add('중립(근거 약함)');
