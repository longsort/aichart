#!/usr/bin/env python3
"""Deploy TF-switch speed + Re-accumulation zone to /root/ailongshort."""
...
def main() -> int:
    ...
    try:
        ...
        run(
            ssh,
            f"grep -n 'mergedDeskReaccumulationZone\\|merged-desk-reacc\\|재매집' "
            f"{REMOTE}/lib/mergedDeskReaccumulationZone.ts "
            f"{REMOTE}/app/components/mergedAnalysis/MergedAnalysisDeskView.tsx | head -n 25",
        )
        ...
        sftp.close()
        ssh.close()
    out("[done] reacc + TF-switch deploy OK -> /root/ailongshort")
    return 0
