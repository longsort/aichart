...
class OrderBookStrengthBoxesV46 extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return FutureBuilder<OrderBook>(
      ...
      builder: (context, snap) {
        if (!snap.hasData) {
          return _card(context, const Text('?§ÎçîÎ∂?Í∞ïÎèÑ: Î°úÎî©...', style: TextStyle(color: Colors.white70)));
        }
        final ob = snap.data!;
        ...
        return _card(
          ...
          Column(
            ...
            children: [
              const Text('?§ÎçîÎ∂?Í∞ïÎèÑ(Í∑ºÏ†ë Í∞ÄÍ≤©Î?)', style: TextStyle(color: Colors.white, fontWeight: FontWeight.w900)),
              const SizedBox(height: 8),
              _rowTitle('ÏßÄÏßÄ(Îß§Ïàò ?ÄÍ∏?', Colors.greenAccent),
              const SizedBox(height: 6),
              _boxes(supports, isSupport: true),
              const SizedBox(height: 10),
              _rowTitle('?Ä??Îß§ÎèÑ ?ÄÍ∏?', Colors.redAccent),
              const SizedBox(height: 6),
              _boxes(resists, isSupport: false),
              const SizedBox(height: 6),
              Text('??Ï≤¥Í≤∞???ÑÎãå ?úÎ?Í∏∞Î¨º???∏Í?)??Í∏∞Î∞ò. (limit=50)', style: TextStyle(color: Colors.white54, fontSize: 11)),
            ],
          ),
        );
      },
  ...
  Widget _boxes(List<_Bucket> buckets, {required bool isSupport}) {
    if (buckets.isEmpty) {
      return const Text('?∞Ïù¥???ÜÏùå', style: TextStyle(color: Colors.white54));
    }
      ...
      children: buckets.map((b) {
        ...
        return Container(
          ...
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            ...
            const SizedBox(height: 6),
            Text('Í∞ïÎèÑ ${pct.toStringAsFixed(0)}%', style: TextStyle(color: color.withOpacity(0.9), fontWeight: FontWeight.w900, fontSize: 12)),
          ]),
        );
      }).toList(),
    );
