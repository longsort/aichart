...
import { subscribeMarketBusCandles } from '@/lib/eagle1/marketDataBus';
...
async function fetchMarketJsonRaw(
  symbol: string,
  timeframe: string,
  source: ClientMarketSource,
  depth?: 'recent' | 'full',
...
/**
 * Phase 1b — ChartView·prefetch가 MarketDataBus로 동일 OHLCV REST를 공유.
 * 공유 중 AbortSignal은 버스 loader에 넘기지 않음(한쪽 abort로 전체 실패 방지).
 */
async function fetchMarketJson(
  ...
  const depthLane = depth === 'recent' ? 'recent' : depth === 'full' ? 'full' : 'default';
  return subscribeMarketBusCandles(
    { symbol, timeframe, source, depth: depthLane },
    () => fetchMarketJsonRaw(symbol, timeframe, source, depth)
  );
}
...
export async function fetchClientMarketCandles(opts: {
  ...
  maxAgeMs?: number;
  /**
   * true면 HTF full 백그라운드 다운로드 안 함 (워밍 전용).
   * 1d/1w/1M 전량이 analyze·현재 차트를 막지 않게 한다.
   */
  ...
  const allowFullUpgrade = opts.skipHtfFullUpgrade !== true;
  ...
  try {
    ...
    if (htf && allowFullUpgrade) addUpgradeListener(key, opts.onUpgrade);
      ...
      pending = (async () => {
        try {
          if (htf) {
            ...
            if (allowFullUpgrade) scheduleHtfFullUpgrade(opts.symbol, tf, source, key);
            return recent;
          }
...
export function prefetchClientMarketCandles(
  ...
  timeframe: string,
  source: ClientMarketSource = 'binance',
  opts?: { skipHtfFullUpgrade?: boolean }
): void {
 ...
 * 이웃 TF는 recent만 (1d/1w/1M full 동시 폭주 → analyze 60s 정체 방지).
 */
export function prefetchMergedDeskAllMarketTfs(
  ...
  ordered.forEach((tf, i) => {
    /** 현재 TF는 즉시(full 허용), 이웃은 recent만 + 더 긴 스태거 */
    const delay = i === 0 ? 0 : 2800 + i * 420;
    window.setTimeout(() => {
      prefetchClientMarketCandles(symbol, tf, source, {
        skipHtfFullUpgrade: i !== 0,
      });
    }, delay);
  });
}
