...
import { pickVisibleSweepEvents } from '@/lib/eagle1Tapoint/sweepLiveSignalTap';
...
export function buildStructureDeskOverlays(params: {
  ...
  /** heat 봉면 오버레이 제거 — HUD `.heatBack`만 유지 (캔들 가림 방지) */
  ...
  /** 연속 SWEEP은 마지막 1개가 아니라 봉마다 핀. SWEEP 단독 주문 아님. */
  const sweepPins = pickVisibleSweepEvents(live, 80, 2);
  for (const ev of sweepPins) {
    const evIdx = Number.isFinite(ev.known_at) ? ev.known_at : ev.index;
    const bar = candles[evIdx] || candles[ev.index];
    const t1 = Number(bar?.time ?? lastTime);
    if (!(t1 > 0)) continue;
    const wick =
      ev.bias === 'bullish'
        ? Number(bar?.low ?? ev.price ?? ev.level)
        : Number(bar?.high ?? ev.price ?? ev.level);
    const px = wick > 0 ? wick : Number(ev.price) || Number(ev.level);
    if (!(px > 0)) continue;
    const id = `eagle1-sweep-pin-${ev.known_at}-${ev.index}`;
    if (seen.has(id)) continue;
    seen.add(id);
    out.push({
      id,
      ...
      overlayZoneExtraClass:
        'eagle1-structure eagle1-structure-mark eagle1-sweep-candle-pin eagle1-hud-keep merged-desk-zone-label-on',
      zoneFaceBase: 'SWEEP',
      labelTooltip: ev.evidence.join(' · ') || '유동성털기 · 단독진입 아님',
    });
  }
