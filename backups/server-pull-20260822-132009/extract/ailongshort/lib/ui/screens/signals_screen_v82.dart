...
import 'package:ailongshort/ui/widgets/bitget_live_header.dart';
import 'package:ailongshort/ui/widgets/multitf_consensus_bar.dart';
import 'package:ailongshort/ui/widgets/ai_conclusion_card.dart';
import 'package:ailongshort/ui/widgets/entry_plan_card.dart';
import 'package:ailongshort/ui/widgets/position_sizing_card.dart';
import 'package:ailongshort/ui/widgets/account_profit_targets_card.dart';
import 'package:ailongshort/ui/widgets/paper_trade_card.dart';
...
import 'package:ailongshort/data/snapshot/snapshot_store.dart';
import 'package:ailongshort/engine/ai/entry_plan.dart';
import 'package:ailongshort/engine/guard/no_trade_guard_store.dart';
import 'package:ailongshort/engine/regime/regime_lock.dart';
import 'package:ailongshort/engine/auto/full_auto_engine.dart';
            ...
            children: [
              ...
              ValueListenableBuilder(
                ...
                builder: (_, t, __) {
                  ...
                  String decision0 = (decisionPack['decision'] ?? 'Í¥ÄÎß?).toString();
...
return Column(
                    children: [
                      _regimeBanner(regime, '${finalPack.reason} ???©Ïùò??$consensusPct%'),
                      const SizedBox(height: 10),
                      AIConclusionCard(
                        decision: decision,
                      ...
                      _recordButton(
                        ...
                        onTap: () async {
                          ...
                          ScaffoldMessenger.of(context).showSnackBar(
                            const SnackBar(content: Text('?Ä???ÑÎ£å')),
                          );
                        },
                      ),
                    ],
  ...
  Widget _noTradeBanner(NoTradeState s) {
    final until = s.until;
    final txt = until == null ? '?†Í∏à' : '?†Í∏à ?¥Ï†ú ${until.toLocal().toString().substring(0, 16)}';
    return Container(
      padding: const EdgeInsets.all(10),
      ...
      child: Row(
        children: [
          ...
          const SizedBox(width: 8),
          Expanded(
            child: Text('?∏Ìä∏?àÏù¥??$txt  (?∞ÏÜç?êÏã§ ${s.lossStreak})',
                style: const TextStyle(color: Colors.white, fontSize: 12, fontWeight: FontWeight.bold)),
          ),
        ],
      ),
  ...
  Widget _recordButton({required bool disabled, required VoidCallback onTap}) {
    return SizedBox(
      ...
      child: ElevatedButton.icon(
        ...
        label: Text(disabled ? '?†Í∏à ?ÅÌÉú' : '?§ÎÉÖ???Ä??),
      ),
    );
  }
}