...
def main():
    ...
    run(
        f"grep -n 'consecutive2\\|candleRsiAdv\\|SWEEP_LIVE_MAX_AGE' "
        f"{remote}/lib/eagle1Tapoint/sweepLiveSignalTap.ts "
        f"{remote}/lib/eagle1Tapoint/signalLiveBriefing.ts | sed -n '1,20p'"
    )
    print("sweep+rsi-adv live OK", flush=True)
    ssh.close()
    return 0
