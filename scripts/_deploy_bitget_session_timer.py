#!/usr/bin/env python3
"""Deploy Bitget session countdown + confirm Bitget candle source."""
...
FILES = [
    ...
    "app/components/ChartCandleCloseTimer.tsx",
    "app/components/TfCandleCloseRemain.tsx",
...
def main() -> int:
    ...
    try:
        ...
        run(
            ssh,
            f"grep -n 'BITGET_HTF_OPEN_HOUR_UTC\\|CandleCloseExchange\\|exchange={{bitgetVolumePackOn' "
            f"{REMOTE}/lib/closeSettlement.ts "
            f"{REMOTE}/app/components/ChartView.tsx | head -n 25",
        )
        ...
        sftp.close()
        ssh.close()
    out("[done] Bitget session timer deploy OK")
    return 0
