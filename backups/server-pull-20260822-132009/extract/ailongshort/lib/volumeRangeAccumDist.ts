...
function windowForTf(tf: string): { win: number; maxSpanPct: number; maxNetPct: number } {
  ...
  const map: Record<string, { win: number; maxSpanPct: number; maxNetPct: number }> = {
    '1m': { win: 18, maxSpanPct: 0.012, maxNetPct: 0.008 },
    '5m': { win: 16, maxSpanPct: 0.014, maxNetPct: 0.01 },
    '15m': { win: 14, maxSpanPct: 0.018, maxNetPct: 0.012 },
    '1h': { win: 12, maxSpanPct: 0.022, maxNetPct: 0.015 },
    '4h': { win: 10, maxSpanPct: 0.038, maxNetPct: 0.022 },
    '1d': { win: 8, maxSpanPct: 0.055, maxNetPct: 0.03 },
    '1w': { win: 6, maxSpanPct: 0.08, maxNetPct: 0.04 },
    '1M': { win: 5, maxSpanPct: 0.12, maxNetPct: 0.05 },
    '1Y': { win: 4, maxSpanPct: 0.18, maxNetPct: 0.07 },
  };
  return map[n] ?? { win: 12, maxSpanPct: 0.03, maxNetPct: 0.018 };
}
...
export function detectVolumeRangePhases(
  ...
  for (let end = start; end < n - 1; end++) {
    ...
    const edge = phase === 'distribution' ? sellPct - 0.5 : 0.5 - buyPct;
