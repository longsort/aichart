...
### `lib/analyze.ts` OB 검증 (TS 차트 엔진)
1. **BOS**: 스윙 BOS/ChoCH + **단순 돌파**(`close > 전봉 high` / `close < 전봉 low`)를 `simpleBos`로 병합해 OB 후보 탐색.
2. **강한 FVG**: 동일 방향 유효 FVG 중 **갭(고저차) ≥ ATR×0.12**만 OB 검증에 사용(약한 이격 제외).
3. **Mitigation**: OB 형성 봉 **이후** 캔들이 구간과 겹치면 `obMitigated`, 라벨 `· 완화`, 색 알파 축소 + 차트 점선 테두리.
