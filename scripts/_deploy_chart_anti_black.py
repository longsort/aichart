...
def main():
    ...
    run(
        f"grep -n 'lastGoodRowsRef\\|autoScale: true\\|TapointKstClock\\|candleFingerprint' "
        f"{remote}/app/components/eagle1Tapoint/TapointCleanChart.tsx "
        f"{remote}/app/components/eagle1Tapoint/Eagle1TapointDeskView.tsx | head -20"
    )
    print("chart anti-black/reset OK", flush=True)
    ssh.close()
    return 0
