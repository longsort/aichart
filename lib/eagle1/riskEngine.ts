...
export function buildRiskPlan(params: {
  ...
  if (dir === 'LONG') {
    ...
    if (!(tp1 > entryMid)) tp1 = entryMid + atr * 1.2;
    tp1Reason = nextPoc && nextPoc > entryMid ? '가까운 최다거래가격' : '근거리 ATR 확장(구조 목표 부족)';
    const tp2CandL = hvnPx && hvnPx > tp1 ? hvnPx : liqHigh;
    tp2 = tp2CandL > tp1 ? tp2CandL : entryMid + atr * 2.4;
    tp2Reason = hvnPx && hvnPx > tp1 ? '다음 거래밀집' : '위쪽 유동성/스윙고점';
    tp3 = mfeR != null ? entryMid + mfeR * riskPx : Math.max(liqHigh, tp2) + atr * 1.6;
    ...
    if (!(tp1 < entryMid)) tp1 = entryMid - atr * 1.2;
    tp1Reason = nextPoc && nextPoc < entryMid ? '가까운 최다거래가격' : '근거리 ATR 확장(구조 목표 부족)';
    /** liqLow가 진입 위면 TP2가 손절쪽으로 붙는 버그 방지 */
    const tp2CandS = hvnPx && hvnPx < tp1 ? hvnPx : liqLow;
    tp2 = tp2CandS < tp1 ? tp2CandS : entryMid - atr * 2.4;
    tp2Reason = hvnPx && hvnPx < tp1 ? '다음 거래밀집' : '아래쪽 유동성/스윙저점';
    tp3 = mfeR != null ? entryMid - mfeR * riskPx : Math.min(liqLow, tp2) - atr * 1.6;
    tp3Reason = mfeR != null ? '표본 MFE 확장' : '확장 목표(측정 이동)';
    if (!(tp3 < tp2)) tp3 = tp2 - atr * 0.8;
  }
