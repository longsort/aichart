...
def main():
    ...
    run(
        f"grep -n 'TAP_ONLY\\|타점전용 · Dual BG\\|isTapointTapOnly' "
        f"{remote}/app/api/merged-desk/live-order/route.ts "
        f"{remote}/lib/mergedDeskDualBgRaceEntry.ts "
        f"{remote}/lib/mergedDeskAutoTradeRunner.ts | sed -n '1,25p'"
    )
    print("tapOnly hard-block OK", flush=True)
    ssh.close()
    return 0
