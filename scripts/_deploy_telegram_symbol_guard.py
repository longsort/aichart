#!/usr/bin/env python3
"""Deploy telegram BTC/ETH price mix guard to /root/ailongshort."""
...
def main() -> int:
    ...
    try:
        ...
        run(
            ssh,
            f"grep -n 'telegramSymbolPriceGuard\\|telegramAssetPricePlausible' "
            f"{REMOTE}/lib/telegramMergedDeskAutoRunner.ts "
            f"{REMOTE}/lib/telegramSymbolPriceGuard.ts | head -n 20",
        )
        ...
        sftp.close()
        ssh.close()
    out("[done] telegram btc/eth price guard deployed")
    return 0
