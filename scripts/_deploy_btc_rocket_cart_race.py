...
FILES = [
    ...
    "app/components/mergedAnalysis/MergedDeskBtcSignalBCard.tsx",
...
def main():
    ...
    run(f"grep -n BTC_ROCKET_CART_SOURCE {remote}/lib/mergedDeskBtcRocketCartSignal.ts | head -2")
    run(f"grep -n btc-rocket-cart {remote}/lib/mergedDeskUnifiedAnalysisEntry.ts | head -5")
    print("OK", flush=True)
    ssh.close()
