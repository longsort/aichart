...
FILES = ["app/components/eagle1Tapoint/Eagle1TapointDeskView.tsx"]
...
def main():
    ...
    def run(cmd, timeout=7200):
        ...
        if out.strip():
            print(safe(out[-2500:] if len(out) > 2500 else out), flush=True)
        if err.strip():
            print(safe(err[-400:]), flush=True)
    ...
    ssh.close()
    print("siglive-top-slim OK", flush=True)
