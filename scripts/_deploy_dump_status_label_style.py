#!/usr/bin/env python3
"""Deploy status colors (대기노랑/하락빨강/상승초록) + borderless small labels."""
...
def main() -> int:
    ...
    try:
        ...
        run(
            ssh,
            f"grep -n '하락확정\\|상승확정\\|transparent\\|fde047' "
            f"{REMOTE}/lib/mergedDeskDumpLifeCycle.ts "
            f"{REMOTE}/app/globals.css | head -n 30",
        )
        ...
        sftp.close()
        ssh.close()
    out("[done] status color + borderless labels OK")
    return 0
