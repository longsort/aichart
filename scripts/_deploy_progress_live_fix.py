...
def main():
    ...
    run(f"grep -n 'enabledList' {remote}/lib/mergedDeskCoinTradeProgress.ts | head -2")
    run(f"grep -n 'Dual초단' {remote}/app/components/mergedAnalysis/MergedAnalysisDeskView.tsx | head -2")
    print("\nOK progress-live deploy", flush=True)
    ssh.close()
    return 0
