...
FILES = [
    "lib/mergedDeskStructureSSignal.ts",
...
def main():
    ...
    run(f"grep -n 'STRUCTURE_S_SOURCE\\|신호C\\|structure-s' {remote}/lib/mergedDeskStructureSSignal.ts {remote}/lib/mergedDeskBtcSignalRace.ts | head -12")
    print("\nOK structure-s race deploy", flush=True)
    ssh.close()
    return 0
