...
/** VRVP 최다거래가 — 차트 전체 노란 가로 점선 */
export function buildMergedVrvpPocPriceLine(poc: number | null | undefined): AtlasPulsePriceLine | null {
  if (poc == null || !Number.isFinite(poc)) return null;
  return {
    price: poc,
    color: '#FDE047',
    title: mergedVrvpPocTitle(poc, fmtPx),
    lineWidth: 2,
    lineStyle: 'dashed',
  };
}

/** VRVP POC + Value Area — 차트 가격축 점선 */
export function buildMergedVrvpPriceLines(
  profile: { poc: number | null; vaHigh: number | null; vaLow: number | null; timeframe?: string } | null | undefined
): AtlasPulsePriceLine[] {
  if (!profile) return [];
...
export function buildMergedDeskPriceLines(
  ...
  vrvp: { poc: number | null; vaHigh: number | null; vaLow: number | null } | null | undefined,
