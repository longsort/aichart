...
FILES = [
    ...
    "lib/serverTapointDecideShare.ts",
...
def main():
    ...
    run(f"mkdir -p {remote}/data/merged-desk-auto-trade-locks {remote}/data/tapoint-accum")
    ...
    run(
        f"grep -n 'paperNoteKo\\|tapOnly === false && !arm.strategyScalp\\|실전만 집계' "
        f"{remote}/lib/mergedDeskCoinSkillBoard.ts "
        f"{remote}/lib/mergedDeskServerAutoTradeRunner.ts "
        f"| sed -n '1,20p'"
    )
    print("fake-stats + tap entry unblock OK", flush=True)
    ssh.close()
    return 0
