...
import 'package:ailongshort/logic/no_trade_lock.dart';
...
class _TopLongShortGaugeState extends State<TopLongShortGauge> {
  ...
  Widget build(BuildContext context) {
    ...
    // - evidence low => ?ÑÌóò Ï¶ùÍ?
    // - direction neutral => ?ÑÌóò Ï¶ùÍ?
    int risk = 40;
    if (_hit <= 3) risk = 85;
  ...
  static String _regimeHint(double score, double conf) {
    final a = score.abs();
    if (a < 15) return 'RANGE ¬∑ Í¥ÄÎßùÍ∂å';
    if (a < 45) return conf >= 0.7 ? 'TREND ¬∑ Ï§ÄÎπ? : '?ºÏ°∞ ¬∑ ?†Ï§ë';
    return conf >= 0.75 ? 'TREND ¬∑ Í∞? : 'TREND ¬∑ Ï£ºÏùò';
  }
}
...
class _GaugeBar extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    // v2: AnimatedAlign Í∏∞Î∞ò?ºÎ°ú Í≤åÏù¥ÏßÄ ?∏ÎîîÏºÄ?¥ÌÑ∞Í∞Ä Î∂Ä?úÎüΩÍ≤??¥Îèô
