#!/usr/bin/env python3
"""Deploy MTF dump bounce set-color + target/prob labels to /root/ailongshort."""
...
def main() -> int:
    ...
    try:
        ...
        run(
            ssh,
            f"grep -n '반등컷\\|dumpTfSetVisual\\|반등확률' "
            f"{REMOTE}/lib/mergedDeskMtfDumpZoneBridge.ts "
            f"{REMOTE}/lib/mergedDeskMtfBounceTargetByTf.ts | head -n 20",
        )
        ...
        sftp.close()
        ssh.close()
    out("[done] dump bounce set-color deploy OK -> /root/ailongshort")
    return 0
