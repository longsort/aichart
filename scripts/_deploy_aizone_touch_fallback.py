...
def main():
    ...
    run(
        f"grep -n 'resolvePendingTouchOrAiZone\\|aizone70\\|AIZONE불일치' "
        f"{remote}/lib/mergedDeskDualStructureEntry.ts "
        f"{remote}/lib/mergedDeskDualBgRaceEntry.ts | head -20"
    )
    print("\nOK aizone-touch-fallback deploy", flush=True)
    ssh.close()
    return 0
