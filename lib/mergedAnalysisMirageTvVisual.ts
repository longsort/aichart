...
export function buildMirageTvStructureOverlays(
  ...
  const hiPair =
    pickSwingPivotPair(swingHighs, tf, 'max') ?? fallbackSwingPivotPair(candles, tf, 'high', start, lastIdx);
  const loPair =
    pickSwingPivotPair(swingLows, tf, 'min') ?? fallbackSwingPivotPair(candles, tf, 'low', start, lastIdx);
