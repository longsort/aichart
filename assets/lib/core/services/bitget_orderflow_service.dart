...
/// STEP 6+7: 비트겟 오더북/체결 기반 오더플로우 요약 → dto(Map)에 주입
/// - dto가 Map이면 자동 동작
/// - dto에 'bitgetApi' 없으면 FuBitgetApi를 자동 생성/주입(공개 API)
class BitgetOrderflowService {
  static Future<void> injectToDto(Map<String, dynamic> dto,
    ...
    // 다양한 응답 형태 대응 (Bitget v2는 data 안에 bids/asks가 들어오는 케이스 많음)
    ...
    if (ratio >= 1.12) bias = '매수우위';
    if (ratio <= 0.88) bias = '매도우위';
    ...
    String liqRisk = '보통';
    if (spreadBp >= 6.0 || topDepth < (bidVol + askVol) * 0.18) liqRisk = '높음';
    if (spreadBp <= 2.0 && topDepth > (bidVol + askVol) * 0.28) liqRisk = '낮음';
    ...
    if (trades != null) {
      ...
      if (t.isNotEmpty) {
        ...
        if (tot > 0) {
          ...
          if (buyP >= 56 && bias != '매도우위') bias = '매수우위';
          if (sellP >= 56 && bias != '매수우위') bias = '매도우위';
        }
      }
    }
