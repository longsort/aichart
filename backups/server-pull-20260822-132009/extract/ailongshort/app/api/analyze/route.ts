...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    const getCandlesForTf = (tf: string) => {
      ...
      if (hit) return hit;
      /** analyze 모드 — 1d/1w/1M 전량 페이지네이션으로 메인스레드·서버 버벅임 방지 */
      const p = fetchMarketCandles(symbol, tf, 'analyze');
      candleByTf.set(tf, p);
      return p;
    };
    ...
    if (useCollect) {
      try {
        ...
        candles = await fetchMarketCandles(symbol, timeframe, 'analyze');
      }
    ...
    const auxTail = (tf: string, n: number) =>
      chartTfNorm === normalizeChartTimeframe(tf)
        ...
        : fetchMarketCandlesTail(symbol, tf, n).catch(() => null);
    const TAIL_D = 160;
    const TAIL_W = 72;
    const TAIL_M = 96;
