...
export function buildUnifiedMarketMetrics(params: {
  ...
  return {
    ...
    liqClusterLongPrice: params.liquidations.clusters?.find((c) => c.side === 'long')?.price ?? null,
    liqClusterShortPrice: params.liquidations.clusters?.find((c) => c.side === 'short')?.price ?? null,
    liqClusterLongUsd: params.liquidations.clusters?.find((c) => c.side === 'long')?.usd ?? 0,
    liqClusterShortUsd: params.liquidations.clusters?.find((c) => c.side === 'short')?.usd ?? 0,
    cmf20,
    exchangeLegs,
