  ...
  if (opts.includeSmcChannel !== false) {
    ...
    if (seg && seg.band1.length >= 2) {
      ...
      if (
        ...
        const tLast = Number(candles[iEnd]!.time);
        const barStep =
          candles.length >= 2
            ? Math.max(
                1,
                Math.abs(Number(candles[iEnd]!.time) - Number(candles[iEnd - 1]!.time))
              )
            : 3600;
        /** 마지막 캔들 +5봉 — 빈 축에 맞춰 면 연장 */
        const t2Ext = Math.max(t2, tLast) + barStep * 5;
        out.push({
          id: `${prefix}channel-active`,
          kind: 'channelBand',
          label: longSeg ? 'Triple·롱' : 'Triple·숏',
          zoneFaceBase: longSeg ? 'Triple·롱' : 'Triple·숏',
          x1: 0,
          y1: 0,
          confidence: 0.55,
          ...
          overlayZoneExtraClass: `merged-tt-fusion-channel merged-desk-zone-label-on ${longSeg ? 'merged-desk-smc-dir-long' : 'merged-desk-smc-dir-short'}`,
        });
      }
    }
  }
