...
class AICsvAutoRun extends ChangeNotifier {
  ...
  Future<void> start({String assetPath = 'assets/data/sample.csv'}) async {
    ...
    if (E >= 0.55) { n++; r.add("?�너지??); }
    if (V <= 0.35) { n++; r.add("변?�성??); }
    if (P >= 0.55) { n++; r.add("방향?�세"); }
    if (R <= 0.45) { n++; r.add("리스?�↓"); }
    if (E >= 0.70) { n++; r.add("거래?�↑"); }
    if (state == MarketState.energy) { n++; r.add("??��준�?); }
    if (state == MarketState.danger) { n++; r.add("?�험경보"); }

    evidenceCount = n.clamp(0, 6);
    reasons = r.take(3).toList();

    // confidence = 증거 기반 + ?�태 보정
