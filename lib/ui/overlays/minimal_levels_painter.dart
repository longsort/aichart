...
/// - ?”ë©´ ì§€?€ë¶„í•¨ ?œê±°??"3?ˆë²¨ + ê¸ˆì?" ?„ìš© ?¤ë²„?ˆì´
/// - ?œì‹œ ê·œì¹™:
///   1) ì¦‰ì‹œ(Immediate): reactLow~reactHigh ?‡ì? ??+ reactLevel ?¼ì¸
///   2) ?¤ìŒ(Target): breakLevel ?¼ì¸
///   3) ?€êµ¬ì¡°(Macro): mbZones ì¤?ê°€????1ê°œë§Œ ?„ì£¼ ?°í•˜ê²?///   4) NO-TRADE(locked): ?ë‹¨ ì¤‘ì•™??'ê¸ˆì?' ë°°ë„ˆ(ì§§ê²Œ)
class MinimalLevelsPainter extends CustomPainter {
  ...
  void _drawTargetLine(Canvas canvas, Size size, double xRight) {
    ...
    _rightTag(canvas, 'ëª©í‘œ', lvl, Offset(xRight - 6, y), theme.warn.withOpacity(0.92));
  }
  ...
  void _drawImmediate(Canvas canvas, Size size, double xRight) {
    ...
    if (lo > 0 && hi > 0 && (hi - lo).abs() > 1e-9) {
      ...
      // ë²”ìœ„ ê°€ê²©ì? ?¤ë¥¸ìª??‘ì? ?œê·¸ 1ê°œë§Œ(ê²¹ì¹¨ ë°©ì?)
      _rightTag(canvas, 'ì¦‰ì‹œ', (lo + hi) * 0.5, Offset(xRight - 6, top + 2), theme.good.withOpacity(0.90));
    }
