...
export function MergedDeskSchematicViewer({
  ...
  const model = useMemo(() => {
    ...
    const nowPx = overlay?.now.price ?? lastPrice ?? pin?.lastPrice ?? pin?.eventPrice;
    ...
    >
      ...
      >
        <div className={styles.schematicHead}>
          <strong>
            결론 · {model.verdict}
            {Number.isFinite(model.nowPx) ? ` · ${formatSchematicPrice(model.nowPx!)}` : ''}
          </strong>
          <button type="button" className={styles.schematicClose} onClick={onClose}>
            닫기
          </button>
        </div>
        <p className={styles.schematicHint}>
          원본은 도식 라벨 위 원만(문자 없음). 우측 글자 크기는 슬라이더로. 확정 아님.
        </p>
        {model.similarKo ? <p className={styles.schematicSimilar}>{model.similarKo}</p> : null}
        {figures.length > 1 && (
          <div className={styles.schematicTabs}>
            {figures.map((f) => (
              <button
                key={f.id}
                type="button"
                className={`${styles.schematicTab} ${fig?.id === f.id ? styles.schematicTabOn : ''}`}
                onClick={() => setFigOverride(f.id)}
              >
                {f.tabKo}
              </button>
            ))}
        ...
        <div className={styles.schematicViewBar}>
          ...
          {viewMode !== 'clean' && (
            <div className={styles.schematicLayerToggles}>
              ...
              <label>
                <input type="checkbox" checked={showLabs} onChange={(e) => setShowLabs(e.target.checked)} />
                약어
              </label>
            </div>
          )}
        ...
        <div className={`${styles.schematicSeatBar} ${sideClass}`}>
          ...
          <span>
            지금가 <strong>{Number.isFinite(model.nowPx) ? formatSchematicPrice(model.nowPx!) : '—'}</strong>
          </span>
          {overlay?.room?.up && (
            <span>
