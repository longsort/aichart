...
import {
  ...
  summarizeCandleEventVerdictKo,
  type CandleEventVerdictPack,
} from '@/lib/mergedDeskCandleEventVerdict';
import type { MtfDumpZonePack } from '@/lib/mergedDeskMtfDumpZoneBridge';
...
import {
  buildCandleCardConfluencePack,
  summarizeCandleCardConfluenceKo,
  type CandleCardConfluencePack,
} from '@/lib/mergedDeskCandleCardConfluence';
import {
  buildMergedDeskSuperStatsHub,
  applySuperStatsHubToActiveTrade,
  type SuperStatsHubPack,
} from '@/lib/mergedDeskSuperStatsHub';
...
import { buildMergedDeskBprOverlays } from '@/lib/bpr';
import {
...
export type MergedAnalysisDeskHud = {
  ...
  /** 실전 AI 플랜 요약 */
  practiceAiPlanKo?: string | null;
  /** 200x 타점 요약 */
  scalp200PlanKo?: string | null;
  /** MTF 폭락구간 요약 */
...
export type MergedAnalysisDeskPack = UnifiedPulseEnginePack & {
  ...
  /** 파랑빨강띠 채널머니 플랜 (텔레·HUD) */
  channelMoneyPlan: MergedDeskChannelMoneyPlan | null;
  /** 파랑빨강띠 채널 지오메트리 */
  channelGeoms: MergedDeskChannelGeom[];
  /** 파랑빨강띠 라이브 진입 허브 */
  rbLiveEntryHub: MergedDeskRbLiveEntryHub | null;
  /** 캔들·카드·마감 합류 팩 (증거) */
  candleCardConfluence: CandleCardConfluencePack;
  /** 超强统计 Hub — 앱 전역 1판정 (카드·캔들·학파는 증거만) */
  superStatsHub: SuperStatsHubPack;
  /** 실전연습 큐 — 자동주문 아님. DeskView 라이브에서 채움 */
  livePracticeCue?: MergedDeskLivePracticeCue;
  /** 실전 AI 플랜 — 안착×합류×ActiveTrade×통계. DeskView 라이브에서 채움 */
  practiceAiPlan?: PracticeAiPlanPack | null;
  /** 200x 타점 — micro-SL · 청산 · ARMED/FIRE. DeskView 라이브에서 채움 */
  scalp200Plan?: import('@/lib/mergedDeskScalp200Plan').Scalp200PlanPack | null;
  /** MTF 폭락구간 팩 */
  mtfDumpPack?: MtfDumpZonePack | null;
  /** REAL CANDLE BATTLE — 실캔들 전투 팩 (DeskView에서 채움) */
  candleBattlePack?: import('@/lib/candleBattle').CandleBattlePack | null;
  /** 캔들 이벤트 — 스윕회수·돌파안착·지지반등 (참고) */
  candleEventVerdict: CandleEventVerdictPack;
  ...
  const pocKo =
    vrvp?.poc != null
      ? mergedVrvpPanelSummary({
          ...
          pocBiasKo: vrvp.pocBiasKo,
          scenarioKo: vrvp.scenarioKo,
          developingPoc: vrvp.developingPoc,
          nearHvnAbove: vrvp.nearHvnAbove,
          nearHvnBelow: vrvp.nearHvnBelow,
        })
...
export function runMergedAnalysisDeskEngine(params: {
  ...
  /** 선택 — MTF 폭락 zone (캔들이벤트 기준선) */
  mtfDumpZones?: import('@/lib/mergedDeskMtfDumpZoneBridge').MtfDumpZoneSpec[] | null;
  ...
  const wavePathOn =
    rbBandOn && loadSettings().chartMergedDeskWavePathEnabled !== false;
  const channelMoneyPack = buildMergedDeskChannelMoneyEdgePack(candles, timeframe, {
    hotZones: hotZoneEntry.all ?? [],
    analysis,
    stanceKey: timeframe,
    waveLockKey: `${String(analysis?.symbol || 'BTC')}:${timeframe}:rb-wave`,
    omitTradeRails: rbBandOn,
    vrvpPoc: tradePack.vrvp?.poc ?? null,
    vrvpVaLow: tradePack.vrvp?.vaLow ?? null,
    vrvpVaHigh: tradePack.vrvp?.vaHigh ?? null,
    wavePathEnabled: wavePathOn,
    coreSr,
    dumpZones: params.mtfDumpZones ?? null,
    keyZones: tradePack.keyZones,
    criticalZones: tradePack.criticalZones,
  });
  const rbLiveEntryHub = rbBandOn
    ? computeMergedDeskRbLiveEntryHub({
        candles,
        geoms: channelMoneyPack.geoms,
        paint: channelMoneyPack.corridorPaint ?? null,
        hotZones: hotZoneEntry.all ?? [],
        vrvpPoc: tradePack.vrvp?.poc ?? null,
        vrvpVaLow: tradePack.vrvp?.vaLow ?? null,
        vrvpVaHigh: tradePack.vrvp?.vaHigh ?? null,
        close: Number(candles[candles.length - 1]?.close) || null,
      })
    : null;

  /** 캔들 분석 · 카드 · 종가마감 → 합류 (ActiveTrade 증거/폴백) */
  const candleCardConfluence = buildCandleCardConfluencePack({
    candles,
    timeframe,
    analysis: analysis ?? null,
    cardPanel,
  ...
  const activeTradePlanRaw = resolveMergedDeskActiveTradePlan({
    candles,
    ...
    candleCardConfluence,
  });

  /** AI超级变身统计 Hub — 전역 1판정. ActiveTrade·가격선·HUD가 이 타점만 사용 */
  const hubClose =
    analysis?.currentPrice ?? bundle.close ?? candles[candles.length - 1]?.close ?? 0;
  const atrArr = candles.length >= 16 ? atrSeries(candles, 14) : [];
  const atrLast = atrArr.length ? atrArr[atrArr.length - 1]! : 0;
  const atrPctHub =
    hubClose > 0 && atrLast > 0 ? (atrLast / hubClose) * 100 : null;
  const superStatsHub = buildMergedDeskSuperStatsHub({
    analysis: analysis ?? null,
    master: masterFutures,
    judgment,
    tradePlan: unifiedTradePlan,
    activeTrade: activeTradePlanRaw,
    hotZone: hotZoneEntry,
    vrvp: tradePack.vrvp,
    mtfAligned: mtfBadge.aligned,
    mtfLabelKo: mtfBadge.labelKo,
    schoolBiasKo:
      judgment.direction === 'LONG'
        ? judgment.stanceKo || '판단 롱편향'
        : judgment.direction === 'SHORT'
          ? judgment.stanceKo || '판단 숏편향'
          : judgment.stanceKo || null,
    confluenceHint: mtfBadge.labelKo,
    cardPanel,
    candleSummaryKo:
      (typeof analysis?.patternVisionSummary === 'string' && analysis.patternVisionSummary) ||
      cardPanel.confirmKo ||
      cardPanel.topsBottomsKo ||
      null,
    recentCandles: candles.slice(-8).map((c) => ({
      open: c.open,
      high: c.high,
      low: c.low,
      close: c.close,
  ...
  const activeTradePlan = applySuperStatsHubToActiveTrade(activeTradePlanRaw, superStatsHub);
  ...
  const candleEventVerdict = buildMergedDeskCandleEventVerdictPack({
    candles,
    timeframe,
    symbol: String(analysis?.symbol || 'BTCUSDT'),
    dumpZones: params.mtfDumpZones ?? null,
    hqZones: hqEntryZones.all,
    keyZones: tradePack.keyZones,
    criticalZones: tradePack.criticalZones,
    rbRails: (channelMoneyPack.geoms ?? []).map((g) => ({
      tipUpper: g.tipUpper,
      tipLower: g.tipLower,
  ...
  const leadingMarks = buildMergedDeskChartMarkers({
    ...
    dumpZones: params.mtfDumpZones ?? null,
    hqZones: hqEntryZones.all,
    candleEventPack: candleEventVerdict,
  });
  ...
  /** 15m ICT BPR — 상승FVG∩하락FVG 주황 균형존 · 재터치 타점 */
  const bprPack = buildMergedDeskBprOverlays({
    candles,
    timeframe,
    maxZones: 2,
  });
  ...
  let chartOverlays = dedupeMergedDeskOverlays([
    ...
    ...(bprPack.overlays.length ? bprPack.overlays : []),
    ...entryZoneOverlays,
    ...(candleCardConfluence.evidenceOverlays.length
      ? candleCardConfluence.evidenceOverlays
      : []),
  ...
  /** 마감·고급 기관밴드 강화 구름 — 통합모드 기관밴드 칩 ON이면 면 표시 */
  if (
    loadSettings().chartMergedInstitutionalBandEnabled !== false &&
    stCloudPack.overlays.length
  ) {
    chartOverlays = dedupeMergedDeskOverlays([...chartOverlays, ...stCloudPack.overlays]);
  }
  ...
  /** $$$$초강·반등 폭증 → 통계 합류 핵심 롱1·숏1 */
  chartOverlays = applyMergedDeskStrongestAnalysisZone(chartOverlays);
  chartOverlays = applyMergedDeskCoreStatsZoneGate(chartOverlays, {
    close: analysis?.currentPrice ?? bundle.close ?? null,
    activeEntry: activeTradePlan.entry > 0 ? activeTradePlan.entry : null,
    bestSupportPx:
      activeTradePlan.direction === 'LONG' && activeTradePlan.entry > 0
        ? activeTradePlan.entry
        : null,
    bestResistPx:
      activeTradePlan.direction === 'SHORT' && activeTradePlan.entry > 0
        ? activeTradePlan.entry
        : null,
    minScore: 58,
  });
  ...
  /** 카드 ↔ ActiveTrade 동일 E/SL/TP (이중 타점 방지) */
  const cardPanelSynced: MergedAnalysisCardPanel = {
    ...cardPanel,
    direction:
      activeTradePlan.direction !== 'NEUTRAL'
        ? activeTradePlan.direction
        : cardPanel.direction,
    entry: activeTradePlan.entry > 0 ? activeTradePlan.entry : cardPanel.entry,
    stopLoss: activeTradePlan.stopLoss > 0 ? activeTradePlan.stopLoss : cardPanel.stopLoss,
    tp1: activeTradePlan.tp1 > 0 ? activeTradePlan.tp1 : cardPanel.tp1,
    tp2: activeTradePlan.tp2 > 0 ? activeTradePlan.tp2 : cardPanel.tp2,
    tp3: activeTradePlan.tp3 > 0 ? activeTradePlan.tp3 : cardPanel.tp3,
    invalidationKo: activeTradePlan.invalidationKo || cardPanel.invalidationKo,
  };
  ...
  return {
    ...
    meta: {
      ...
      }${candleCardConfluence.linked ? ` · ${candleCardConfluence.summaryKo}` : ''}`,
      strategy: {
        ...strategy,
    ...
    activeTradePlan,
    channelMoneyPlan: channelMoneyPack.plan,
    channelGeoms: channelMoneyPack.geoms ?? [],
    rbLiveEntryHub,
    candleCardConfluence,
    candleEventVerdict,
    superStatsHub,
    ...
    deskHud: {
      ...
      candleCardConfluenceKo: summarizeCandleCardConfluenceKo(candleCardConfluence) || null,
      candleEventVerdictKo: summarizeCandleEventVerdictKo(candleEventVerdict) || null,
      rbLiveEntryKo: rbLiveEntryHub?.headlineKo ?? null,
      rbLiveEntryDetailKo: rbLiveEntryHub?.detailKo ?? null,
      rbLiveEntryActionKo: rbLiveEntryHub?.actionKo ?? null,
      rbLiveEntryGradeKo: rbLiveEntryHub?.gradeKo ?? null,
      rbVolumeTagKo: rbLiveEntryHub?.volumeTagKo ?? null,
      rbVolumeStoryKo: rbLiveEntryHub?.volumeStoryKo ?? null,
      rbVolumeGateKo: rbLiveEntryHub?.volumeGateKo ?? null,
    },
    swingDrawEnabled,
  };
}
