...
FILES = [
    "lib/eagle1Tapoint/sweepLiveSignalTap.ts",
...
def main():
    ...
    run(
        f"grep -n 'SWEEP_CONSEC_SKILL\\|sweepConsecSkillMeta\\|3봉내연속스윕진입\\|skill:sweep-consec' "
        f"{remote}/lib/eagle1Tapoint/sweepLiveSignalTap.ts "
        f"{remote}/lib/eagle1Tapoint/signalLiveBriefing.ts "
        f"{remote}/lib/mergedDeskServerAutoTradeRunner.ts "
        f"{remote}/lib/mergedDeskLiveEntryTelegram.ts "
        f"| sed -n '1,30p'"
    )
    print("sweep consec skill + TG OK", flush=True)
    ssh.close()
    return 0
