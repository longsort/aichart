...
export function getChartPriceScaleLayout(opts: {
  ...
  const merged = Boolean(opts.mergedDesk);
  const compact = Boolean(opts.mobileFs || opts.chartFullscreen || merged);
  const userFs =
    opts.axisFontSize != null && Number.isFinite(opts.axisFontSize)
      ? Math.max(8, Math.min(16, Math.round(opts.axisFontSize)))
      : null;
  const layoutFontSize = userFs ?? (compact ? 11 : undefined);
  /** 통합·분석: PC·폰 같은 축폭·우측여백 — 폰만 줄이면 가시 봉 비율이 달라짐 */
  const minimumWidth =
    layoutFontSize != null
      ? Math.max(32, Math.min(78, Math.round(layoutFontSize * 4.2 + 10)))
      : compact
        ? 46
        : 58;
  const mergedFuturePad = merged ? MERGED_DESK_RIGHT_FUTURE_BARS : 0;
  return {
    minimumWidth,
    rightOffset: merged
      ? mergedFuturePad
...
/**
 * 전체화면/폰: 거래량(히스토그램·선진거래량 스택·마커)이 보이도록 하단 패널 확보.
 * 예전 폰 FS top:0.978(~2%)는 막대·마커가 사실상 안 보였음.
 */
export function getVolumePanelLayout(
  fullscreen: boolean,
  mobileFs = false,
  mergedDesk = false
): VolumePanelLayout {
  if (mergedDesk) {
    /** 통합·분석 — PC·폰·전체화면 동일 캔들/거래량 비율 */
    return {
      candleScaleMargins: { top: 0.08, bottom: 0.28 },
      volumeScaleMargins: { top: 0.74, bottom: 0 },
      volPanelTopRatio: 0.74,
    };
  }
  if (mobileFs) {
    return {
      candleScaleMargins: { top: 0.04, bottom: 0.22 },
      volumeScaleMargins: { top: 0.78, bottom: 0.01 },
      volPanelTopRatio: 0.78,
    };
  }
  if (fullscreen) {
    return {
      candleScaleMargins: { top: 0.06, bottom: 0.16 },
      volumeScaleMargins: { top: 0.84, bottom: 0.01 },
      volPanelTopRatio: 0.84,
    };
  }
  return {
    candleScaleMargins: { top: 0.1, bottom: 0.22 },
    volumeScaleMargins: { top: 0.82, bottom: 0.04 },
    volPanelTopRatio: 0.82,
  };
}

/** LWC 같은 차트 pane — 캔들(우측 축) + 거래량(overlay 히스토그램) 맨 아래 공동 */
export function applyChartVolumePanelLayout(
  chart: import('lightweight-charts').IChartApi,
  layout: VolumePanelLayout,
  series?: {
    volume?: import('lightweight-charts').ISeriesApi<'Histogram'> | null;
    volumeBuy?: import('lightweight-charts').ISeriesApi<'Histogram'> | null;
    volumeMa?: import('lightweight-charts').ISeriesApi<'Line'> | null;
  }
): void {
  const volScaleOpts = {
    scaleMargins: layout.volumeScaleMargins,
    autoScale: true,
    visible: true,
    borderVisible: false,
  };
  const applyOnce = () => {
    try {
      chart.applyOptions({
        rightPriceScale: {
          scaleMargins: layout.candleScaleMargins,
          visible: true,
        },
        overlayPriceScales: {
          scaleMargins: layout.volumeScaleMargins,
    ...
    try {
      chart.priceScale('right').applyOptions({ scaleMargins: layout.candleScaleMargins });
    } catch {
      /* ignore */
    }
    try {
      chart.priceScale('').applyOptions(volScaleOpts);
    } catch {
      /* ignore */
    }
    for (const s of [series?.volume, series?.volumeBuy, series?.volumeMa]) {
      if (!s) continue;
      try {
        s.priceScale().applyOptions(volScaleOpts);
      } catch {
        /* ignore */
      }
    }
    try {
      series?.volume?.applyOptions({ visible: true, base: 0, lastValueVisible: false, priceLineVisible: false });
      /** buy 시리즈는 데이터 없으면 숨김 — 빈 overlay가 스케일을 깨지 않게 */
      const buy = series?.volumeBuy;
      if (buy) {
        buy.applyOptions({ visible: true, base: 0, lastValueVisible: false, priceLineVisible: false });
      }
      series?.volumeMa?.applyOptions({ visible: true, lastValueVisible: false });
    } catch {
      /* ignore */
    }
  };
...
export function buildVolumeSegmentMarkers(
  ...
  for (const z of pack.zones.filter((x) => x.layer === 'cluster')) {
    ...
    out.push({
      ...
      text: clamp(buildVolumeVisibleLabel(z, 80), 10),
      size: 1,
    });
  }
  ...
  for (const z of pack.zones.filter((x) => x.layer === 'trend' && x.bars >= engine.pivotMinBars)) {
    ...
    out.push({
      ...
      shape: 'square',
      color: z.verdictColor,
      text: clamp(buildVolumeVisibleLabel(z, 56), 10),
      size: 1,
    });
  }
