...
class _Sheet extends StatelessWidget {
  ...
  String _stateText() {
    switch (state) {
      case MarketState.stable: return "稳定/?�정";
      case MarketState.energy: return "?�量/?�너지";
      case MarketState.uncertain: return "不确�?불안";
      case MarketState.danger: return "?�险/?�험";
    }
  }
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          const Text("?�来/미래", style: TextStyle(color: Colors.white70, fontSize: 12, fontWeight: FontWeight.w700)),
          ...
          const SizedBox(height: 12),
          const Text("简�??�약", style: TextStyle(color: Colors.white70, fontSize: 12, fontWeight: FontWeight.w700)),
          const SizedBox(height: 6),
  ...
  String _oneLine() {
    if (state == MarketState.danger) return "风险?�高，先观望/리스???�음 관�?;
    if (state == MarketState.uncertain) return "?��??�显，等待更好点�?불확???��?;
    if (state == MarketState.energy) return "?�量?�集，关注回�??�너지 축적 ?�림 주시";
    return "?�态稳定，??��虑进入/?�정 진입 고려";
  }
}
