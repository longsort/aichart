...
import { computeMergedDeskRbEdgeConfluenceGate } from '@/lib/mergedDeskRbEdgeConfluenceGate';
...
export function computeMergedDeskRbFullConfluence(params: {
  ...
  /** LOCK 레일 합류 게이트 — GP·AVWAP·VP·SFP */
  const primaryForGate =
    params.geoms?.find((g) => g.primary) ??
    params.geoms?.find((g) => g.horizon === 'short') ??
    params.geoms?.[0] ??
    null;
  const edgeGate =
    ...
    computeMergedDeskRbEdgeConfluenceGate({
      ...
      geom: primaryForGate,
      volSync: params.volSync,
      avwapHigh: params.avwapHigh,
      avwapLow: params.avwapLow,
      ...
      moneyHud: params.moneyHud,
      analysis: params.analysis,
      wavePhase: params.wavePhase,
      masterSide: side,
    });
  if (edgeGate.placeRefOk) {
    score += add(
      reasons,
      `레일합류 ${edgeGate.shortKo} · 핵심${edgeGate.coreHitCount}/6`,
      14 + Math.min(8, edgeGate.coreHitCount * 2),
      true
    );
    aligned += 1;
  } else if (edgeGate.coreHitCount >= 1) {
    score += add(reasons, `레일합류 부분 · 핵심${edgeGate.coreHitCount}/6`, 4, false);
  } else {
    score += add(reasons, '레일합류 부족 · 자리대기', -4, false);
    conflict += 1;
  }
  for (const h of edgeGate.hits.filter((x) => x.ok).slice(0, 5)) {
    score += add(reasons, h.labelKo, Math.min(6, Math.round(h.weight / 3)), true);
    aligned += 1;
  }
  ...
  const hardWait =
    ...
    (!edgeGate.placeRefOk && edgeGate.coreHitCount === 0);

  let grade: RbConfluenceGrade = 'WAIT';
  let entryAllowed = false;
  if (!hardWait && side !== 'WAIT') {
    if (score >= 78 && aligned >= 6 && conflict === 0 && edgeGate.placeRefOk) {
      grade = 'A';
      entryAllowed = plan?.entryAllowed !== false && plan?.status !== 'WAIT';
    } else if (score >= 62 && aligned >= 4 && conflict <= 1 && edgeGate.placeRefOk) {
      grade = 'B';
      entryAllowed = false;
    } else if (score >= 42) {
      grade = 'C';
  ...
  const gateTag = edgeGate.placeRefOk ? '·자리참고' : '';
  const waveTag =
    params.wavePhase === 'LOCK'
      ...
      : params.wavePhase === 'FREEZE'
        ...
        : params.wavePhase === 'REBUILD'
          ? '·재구축'
          : '';
  const shortKo = grade === 'WAIT' ? `합류대기·${dirKo}${waveTag}` : `합류${grade}·${dirKo}${gateTag}${waveTag}`;
  const summaryKo = `${shortKo} ${score}점 · 동의${aligned} 충돌${conflict} · ${edgeGate.summaryKo} · ${inv} · 합류점수(승률 아님)`;
  ...
  return {
    ...
    reasons,
    edgeGate,
    wavePhase: params.wavePhase ?? null,
  };
}
