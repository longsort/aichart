#!/usr/bin/env python3
"""Full sync to /root/ailongshort — exclude heavy junk, keep remote .env.local + Bitget keys, build+pm2."""
...
EXCLUDE_TOP = {
    ...
    "backups",
    "agent-transcripts",
    # 전체 assets 제외 (무거움·서버 런타임 불필요) — 사용자 요청
    "assets",
    "components/wms",
    "lib/wms",
    "app/(wms)",
...
EXCLUDE_PREFIXES = (
    ...
    # 서버에만 있는 Bitget API 키 — 로컬 빈/다른 키로 덮어쓰지 않음
    "data/exchange-keys/",
)
...
def should_skip(name: str) -> bool:
    ...
    for p in EXCLUDE_PREFIXES:
        if n == p.rstrip("/") or n.startswith(p):
            return True
    # never pack .next even if walk races
    if "/.next/" in f"/{n}/" or n == ".next" or n.startswith(".next/"):
        return True
...
def main() -> int:
    ...
    try:
        ...
        try:
            ...
            # 원격 .env.local 백업 — tar에 로컬 env가 섞여도 운영 시크릿 유지
            run(
                ssh,
                f"test -f {REMOTE_DIR}/.env.local && "
                f"cp -a {REMOTE_DIR}/.env.local /tmp/ailongshort.env.local.bak || true",
            )
            # Bitget API 키(사용자별 암호화 파일) — 배포 후에도 재인증 불필요
            run(
                ssh,
                f"rm -rf /tmp/ailongshort.exchange-keys.bak && "
                f"if [ -d {REMOTE_DIR}/data/exchange-keys ]; then "
                f"cp -a {REMOTE_DIR}/data/exchange-keys /tmp/ailongshort.exchange-keys.bak && "
                f"echo '[keys] backed up remote exchange-keys'; "
                f"else echo '[keys] no remote exchange-keys yet'; fi",
            )
            ...
            run(
                ...
                f"if [ -f /tmp/ailongshort.env.local.bak ]; then "
                f"cp -a /tmp/ailongshort.env.local.bak {REMOTE_DIR}/.env.local && "
                f"chmod 600 {REMOTE_DIR}/.env.local && "
                f"echo '[env] restored remote .env.local'; "
                f"elif [ -f .env.local ]; then echo '[env] .env.local present'; "
                f"else echo '[env] WARN missing .env.local'; fi && "
                f"mkdir -p {REMOTE_DIR}/data && "
                f"if [ -d /tmp/ailongshort.exchange-keys.bak ]; then "
                f"rm -rf {REMOTE_DIR}/data/exchange-keys && "
                f"cp -a /tmp/ailongshort.exchange-keys.bak {REMOTE_DIR}/data/exchange-keys && "
                f"chmod -R go-rwx {REMOTE_DIR}/data/exchange-keys && "
                f"echo '[keys] restored remote exchange-keys'; "
                f"else echo '[keys] skip restore (no backup)'; fi",
            )
