...
import { BNB_SFP_SYMBOL } from '@/lib/mergedDeskBnbSfpTrade';
import { BNB_SFP_YEAR_TFS, runBnbSfpYearReplay } from '@/lib/doksuri1/bnbSfpYearReplay';
...
async function loadYear(tf: string, force: boolean): Promise<{ candles: Candle[]; source: string }> {
  ...
  if (!force) {
    try {
      const csv = await readBitgetFuturesCsv(BNB_SFP_SYMBOL, tf);
  ...
  const live = await fetchBitgetFuturesCandlesFullHistory(BNB_SFP_SYMBOL, tf, target);
  if (live.length >= 80) {
    try {
      await writeBitgetFuturesCsv(BNB_SFP_SYMBOL, tf, live);
    } catch {
      /* */
    }
  }
...
export async function GET(req: NextRequest) {
  ...
  for (const tf of BNB_SFP_YEAR_TFS) {
    try {
  ...
  if (!byTf.length) {
    return NextResponse.json({ ok: false, error: 'BNB 캔들 없음', errors }, { status: 500 });
  }

  const pack = runBnbSfpYearReplay({ byTf, leverage });
