...
class ActivationStatusBar extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final watch = dp < 20.0;
    final modeTxt = isFutureMode ? 'AI ?ˆì¸¡' : '?¤ì „';

    final stateTxt = isLocked ? '? ê¸ˆ' : (watch ? 'ê´€ë§? : '?œì„±');
      ...
      child: SingleChildScrollView(
        ...
        child: Row(
          ...
          children: [
            ...
            _dot(text: '?œì‹œ', on: FeatureFlags.enableFutureProjectionOverlay || FeatureFlags.enableZoneProbLabels || FeatureFlags.enableEntryMarkers),
            const SizedBox(width: 6),
            _dot(text: 'DB', on: FeatureFlags.enableSqliteTradeLogs),
            const SizedBox(width: 6),
            _dot(text: '?ì •', on: FeatureFlags.enableAutoJudge),
            const SizedBox(width: 6),
            _dot(text: '?ì¤‘', on: FeatureFlags.enableRollingHitRate),
            const SizedBox(width: 6),
            _dot(text: '??, on: FeatureFlags.enableNoTradeLock),
            const SizedBox(width: 6),
            _dot(text: '?œë‹', on: FeatureFlags.enableAutoTune),
          ],
        ),
      ),
    );
