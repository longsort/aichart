...
FILES = [
    "lib/eagle1Tapoint/symbolLevTp.ts",
...
def main():
    ...
    run(
        f"grep -n 'resolveTapointSymbolLevTp\\|BTC·ETH 20x' "
        f"{remote}/lib/eagle1Tapoint/symbolLevTp.ts "
        f"{remote}/lib/mergedDeskServerAutoTradeRunner.ts | head -15"
    )
    print("\nOK tapoint symbol lev/tp deploy", flush=True)
    ssh.close()
    return 0
