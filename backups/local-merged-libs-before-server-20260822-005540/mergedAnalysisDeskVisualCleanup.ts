...
export function isMergedDeskRequestedVisibleZone(o: OverlayItem): boolean {
  ...
  if (id.startsWith('eagle1-') || extra.includes('eagle1-zone')) return true;
  if (kind === 'channelBand') {
    return (
  ...
  if (id.startsWith('merged-desk-hq-') || extra.includes('merged-hq-entry')) return true;
  ...
  if (id.startsWith('merged-ares-mlsp-tv-') && /support|resist|fvg|confluence|zone/i.test(id)) return true;
  ...
  const polished = base.map((raw, i) => {
      ...
      const monthTight =
        Number.isFinite(p1) && Number.isFinite(p2) && mid > 0
          ...
          : null;
      const frozenTop = Number(raw.priceFrozen1);
      const frozenBot = Number(raw.priceFrozen2);
      const tight =
        Number.isFinite(frozenTop) && Number.isFinite(frozenBot)
          ? { top: frozenTop, bot: frozenBot }
          : monthTight && mid > 0
      ...
      return {
        ...
        priceFrozen1: tight ? tight.top : raw.priceFrozen1,
        priceFrozen2: tight ? tight.bot : raw.priceFrozen2,
