#!/usr/bin/env python3
"""Deduplicate PM2 ailongshort processes after bad ||/&& deploy chain."""
...
def run(ssh: paramiko.SSHClient, cmd: str, timeout: int = 120) -> None:
    out(f"$ {cmd}")
...
def main() -> int:
    ...
    try:
        ...
        run(
            ssh,
            f"(test -f {REMOTE_DIR}/ecosystem.config.cjs && "
            f"pm2 start {REMOTE_DIR}/ecosystem.config.cjs) || "
            f"(test -f {REMOTE_DIR}/ecosystem.config.js && "
