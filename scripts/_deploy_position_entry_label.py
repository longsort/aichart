...
def main():
    ...
    run(
        f"grep -n 'entryMemos\\|writeServerPositionEntryMemo\\|formatPositionEntrySignalKo\\|rememberPositionEntryLabel' "
        f"{remote}/lib/serverMergedDeskAutoTradeStore.ts "
        f"{remote}/lib/mergedDeskServerArmClient.ts "
        f"{remote}/app/components/mergedAnalysis/MergedDeskAutoTradePanel.tsx "
        f"| sed -n '1,25p'"
    )
    print("position entry signal label OK", flush=True)
    ssh.close()
    return 0
