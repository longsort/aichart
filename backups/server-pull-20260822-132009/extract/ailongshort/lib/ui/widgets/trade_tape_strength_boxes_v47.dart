...
class _TradeTapeStrengthBoxesV47State extends State<TradeTapeStrengthBoxesV47> {
  ...
  static const double _fixedHeight = 260; // 로딩/?�환 ???�면??밀리는 ?�상 방�?
    ...
    return SizedBox(
      ...
      builder: (ctx, snap) {
        ...
        if (fills.isEmpty || widget.lastPrice <= 0) {
          return _card(const Text('체결 ?�이???�음', style: TextStyle(color: Colors.white70)));
        }
        ...
        if (bands.isEmpty) {
          return _card(const Text('체결 밴드 ?�음', style: TextStyle(color: Colors.white70)));
        }
        ...
        return _card(
          Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            const Text('체결 강한 가격�?(최근 100체결)',
                style: TextStyle(color: Colors.white, fontWeight: FontWeight.w900)),
            const SizedBox(height: 10),
            Row(children: [
              Expanded(child: _sideBox(title: 'BUY ?�세', items: buyTop, isBuy: true)),
              const SizedBox(width: 10),
              Expanded(child: _sideBox(title: 'SELL ?�세', items: sellTop, isBuy: false)),
            ]),
            const SizedBox(height: 6),
            Text('밴드 ?? ??${(widget.lastPrice * 0.001).toStringAsFixed(1)}',
        ...
        child: Column(
          ...
          children: [
            ...
            Expanded(
              child: Row(
                children: [
                  Expanded(child: _skeletonSide('BUY ?�세')),
                  const SizedBox(width: 12),
                  Expanded(child: _skeletonSide('SELL ?�세')),
                ],
              ),
            ),
          ],
