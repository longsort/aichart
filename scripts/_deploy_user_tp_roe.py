...
def main():
    ...
    run(
        f"grep -n 'scalpTp1RoePct: tp1\\|opts.tp1RoePct\\|cfg.scalpTp1RoePct' "
        f"{remote}/lib/mergedDeskAutoTradeConfig.ts "
        f"{remote}/lib/mergedDeskServerLiveRoeExit.ts "
        f"{remote}/lib/mergedDeskLiveRoeExit.ts | head -15"
    )
    print("\nOK user-tp-roe deploy", flush=True)
    ssh.close()
    return 0
