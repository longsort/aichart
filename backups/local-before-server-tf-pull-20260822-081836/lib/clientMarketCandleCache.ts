 ...
 *
 * 전 TF: recent(엔진 충분 길이) 즉시 → full 백그라운드 (기능·전량 유지).
...
function scheduleFullUpgrade(
  ...
  if (htf) scheduleHtfFullUpgrade(symbol, tf, source, cacheKey);
  else scheduleBackgroundRefresh(symbol, tf, source, false, cacheKey);
}
...
export async function fetchClientMarketCandles(opts: {
  ...
  try {
    ...
    if (liveMaxAge != null && hit?.candles.length) {
      ...
      /** recent만 있음 — 즉시 표시 + full 백그라운드 (전 TF, 엔진 충분 길이) */
      opts.onPartial?.(hit.candles);
      addUpgradeListener(key, opts.onUpgrade);
      scheduleFullUpgrade(opts.symbol, tf, source, key, htf);
      return hit.candles;
    }
    ...
    if (!pending) {
      pending = (async () => {
        try {
          /** 전 TF: recent 즉시 — 전량 페이지네이션 대기 제거 (기능은 full로 유지) */
