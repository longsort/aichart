...
def main():
    ...
    run(
        f"grep -n 'Parallel Pivot\\|merged-desk-ppl\\|피벗선합류' "
        f"{remote}/lib/mergedDeskParallelPivotLines.ts "
        f"{remote}/lib/mergedDeskDualStructureEntry.ts | head -20"
    )
    print("\nOK parallel-pivot-lines deploy", flush=True)
    ssh.close()
    return 0
