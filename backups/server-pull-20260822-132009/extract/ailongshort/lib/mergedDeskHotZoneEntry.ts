    ...
    if (slipped) {
      ...
      const kept: MergedDeskHotZoneEntry = next
        ...
        : {
            ...
            reasonKo: '안착유지',
            primary: side === 'LONG',
            touchedNow: price >= prev.bot && price <= prev.top,
            status: price >= prev.bot && price <= prev.top ? 'TOUCH' : 'WAIT',
            statusKo: price >= prev.bot && price <= prev.top ? '터치' : '대',
            sources: ['HotZone'],
            time1: prev.time1,
          };
      hzLocks.set(key, {
...
export function buildMergedDeskHotZoneBandPriceLines(
  ...
  const pushBand = (z: MergedDeskHotZoneEntry, isLong: boolean) => {
    ...
    const edgeColor = isLong ? 'rgba(74,222,128,0.5)' : 'rgba(248,113,113,0.5)';
    const st =
    ...
    if (z.top > z.mid && Math.abs(z.top - z.mid) / z.mid > 0.00025) {
      out.push({
        price: z.top,
        color: edgeColor,
        title: `${tag}상`,
    ...
    if (z.bot < z.mid && Math.abs(z.mid - z.bot) / z.mid > 0.00025) {
      out.push({
        price: z.bot,
        color: edgeColor,
        title: `${tag}하`,
...
function toEntry(
  ...
  const touchedNow = price >= c.bot && price <= c.top;
  return {
...
export function buildMergedDeskHotZoneEntryPack(params: {
  ...
  const maxH = Math.max(atrVal * 0.72, price * 0.0048);
  ...
  const huntBelow = bestBelow ? tightenHuntBand(bestBelow, atrVal, price) : null;
  const huntAbove = bestAbove ? tightenHuntBand(bestAbove, atrVal, price) : null;

  const belowRaw = huntBelow
    ? toEntry('LONG', huntBelow, price, primarySide === 'LONG' || primarySide === 'WAIT')
    : null;
  const aboveRaw = huntAbove
    ? toEntry('SHORT', huntAbove, price, primarySide === 'SHORT')
    : null;
  const below = persistHotZoneBand(tf, 'LONG', belowRaw, price, atrVal);
  const above = persistHotZoneBand(tf, 'SHORT', aboveRaw, price, atrVal);
  ...
  for (const z of all) {
    resolveStatus(z, price, params.swingMidEntry, params.masterFutures ?? null);
  }
