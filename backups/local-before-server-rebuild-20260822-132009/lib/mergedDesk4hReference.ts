 ...
 * 1d/1w/1M 차트 setData도 화면용 cap(아래 DISPLAY_CAP) — 전량 sanitize는 TF 전환을 멈춤.
 */
...
/** 차트 setData용 — 일·주·월도 화면용 cap(전량 sanitize·setData는 TF 전환을 멈춤) */
const MERGED_DESK_HTF_CHART_DISPLAY_CAP: Record<string, number> = {
  '1d': 900,
  '1w': 520,
  '1M': 360,
  '1Y': 240,
...
export function mergedDeskChartDisplayCandles(candles: Candle[], timeframe: string): Candle[] {
  ...
  const tf = normalizeChartTimeframe(timeframe);
  if (isListingFullHistoryTf(tf)) {
    const cap = MERGED_DESK_HTF_CHART_DISPLAY_CAP[tf] ?? 900;
    if (candles.length > cap) return candles.slice(-cap);
    return candles;
  }
