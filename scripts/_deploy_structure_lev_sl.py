...
def main():
    ...
    run(f"grep -n 'resolveStructureAwareLevSlTp\\|구조SL유지' {remote}/lib/mergedDeskLiveSlTp.ts {remote}/lib/mergedDeskUnifiedAnalysisEntry.ts | head -10")
    print("\nOK structure-lev-sl deploy", flush=True)
    ssh.close()
    return 0
