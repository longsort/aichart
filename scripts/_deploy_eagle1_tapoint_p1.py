...
def main():
    ...
    run(
        f"grep -n 'runTapCausalSimilarity\\|runExtremeEventEngine\\|advanceTapEntryState' "
        f"{remote}/lib/eagle1Tapoint/orchestrator.ts | head -20"
    )
    print("\nOK eagle1 tapoint P1 deploy", flush=True)
    ssh.close()
    return 0
