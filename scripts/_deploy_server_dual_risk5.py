...
def main():
    ...
    run(
        f"grep -n 'collectDual\\|DUAL_SERVER_SYMS\\|resolveAccountRiskMarginUsdt\\|구조SL유지' "
        f"{remote}/lib/mergedDeskServerAutoTradeRunner.ts "
        f"{remote}/lib/mergedDeskLiveSlTp.ts | head -20"
    )
    print("\nOK server-dual-race risk5 deploy", flush=True)
    ssh.close()
    return 0
