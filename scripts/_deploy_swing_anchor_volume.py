#!/usr/bin/env python3
"""Deploy swing-anchor big long/short volume events upgrade."""
...
def main() -> int:
    ...
    try:
        ...
        run(
            ssh,
            f"grep -n '빅롱 V' {REMOTE}/lib/mergedDeskSwingAnchorVolumeEvents.ts | head -2",
        )
        ...
        out("[done] swing-anchor volume upgrade OK -> /root/ailongshort")
    finally:
        sftp.close()
        ssh.close()
    return 0
