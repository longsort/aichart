#!/usr/bin/env python3
"""Deploy dump zone touch count + bounce prob to app + telegram."""
...
FILES = [
    "lib/mergedDeskDumpZoneTouchStats.ts",
...
def main() -> int:
    ...
    try:
        ...
        run(
            ssh,
            f"grep -n 'touchProbLineKo\\|computeDumpZoneTouchStats\\|터치' "
            f"{REMOTE}/lib/telegramDumpPathBrief.ts "
            f"{REMOTE}/lib/mergedDeskDumpZoneTouchStats.ts | head -n 20",
        )
        ...
        sftp.close()
        ssh.close()
    out("[done] dump touch+prob telegram/app deploy OK -> /root/ailongshort")
    return 0
