...
class DashboardScreenV82 extends StatelessWidget {
  ...
  Color _bgByBias(double bias) {
    if (bias >= 0.10) return const Color(0xFF00FF7A).withOpacity(0.10); // 연한 초록
    if (bias <= -0.10) return const Color(0xFFFF2D55).withOpacity(0.10); // 연한 빨강
    return Colors.transparent;
  }

  String _dir(double bias) => bias > 0.10 ? '롱' : (bias < -0.10 ? '숏' : '중립');
  ...
  Widget build(BuildContext context) {
    return StreamBuilder<EngineSnapshot>(
      ...
      builder: (context, snap) {
        ...
        // pwr=매수/매도 힘, vol=변동, whale=큰손, pat=패턴
        ...
        return Scaffold(
          appBar: AppBar(title: Row(children: const [Text('대시보드')]), actions: [ValueListenableBuilder<String>(valueListenable: SymbolController.I.symbol, builder: (context, s, _) { return DropdownButtonHideUnderline(child: DropdownButton<String>(value: s, dropdownColor: Colors.black, items: const [DropdownMenuItem(value: 'BTCUSDT', child: Text('BTC')), DropdownMenuItem(value: 'ETHUSDT', child: Text('ETH')), DropdownMenuItem(value: 'SOLUSDT', child: Text('SOL'))], onChanged: (v) { if (v!=null) SymbolController.I.set(v); },)); })]),
          body: BiasBackground(
        child: Container(
  ...
  static String _priceTextFromSnapshot(EngineSnapshot s) {
    // 가격은 별도 스토어에서 관리되지만, 대시보드에서는 "값이 없으면 --"로 둔다.
    // (실가격 표시는 홈/신호 화면에서 이미 가능)
    return '--';
  }
}
    ...
    return _PanelCard(
      ...
      child: Column(
        ...
        children: [
          ...
          Center(
            child: Text(
              '롱 ${longP.toStringAsFixed(0)}%  /  숏 ${shortP.toStringAsFixed(0)}%',
    ...
    return ValueListenableBuilder<List<SignalLogEntry>>(
      ...
      builder: (context, entries, _) {
        ...
        for (final e in entries) {
          if (e.decision.contains('롱')) longCount++;
          if (e.decision.contains('숏')) shortCount++;
        }
        ...
        return _PanelCard(
          title: '진입 로그 요약',
          badge: '총 ${entries.length}개',
          child: Column(
            crossAxisAlignment: CrossAxisAlignment.stretch,
            children: [
              _KeyValueRow(label: '롱 신호', value: '$longCount 개'),
              const SizedBox(height: 8),
              _KeyValueRow(label: '숏 신호', value: '$shortCount 개'),
              const SizedBox(height: 12),
              ...
              Text('근거 Top', style: TextStyle(fontWeight: FontWeight.w800, color: Colors.white.withOpacity(0.9))),
              const SizedBox(height: 8),
              _EvidenceTopList(),
              const SizedBox(height: 12),
            ...
            child: Row(
              children: [
                ...
                Text(
                  ...
                  style: TextStyle(
                    fontWeight: FontWeight.w800,
                    color: e.decision.contains('롱')
                        ? const Color(0xFF00FF7A)
                        : (e.decision.contains('숏') ? const Color(0xFFFF2D55) : Colors.white),
                  ),
                ),
              ],
            ),
...
class _HalfGaugePainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    // labels
    _drawText(canvas, '숏', Offset(cx - r * 0.92, cy - r * 0.45), 13, Colors.white.withOpacity(0.70));
    _drawText(canvas, '중립', Offset(cx, cy - r * 1.02), 12, Colors.white.withOpacity(0.55), center: true);
    _drawText(canvas, '롱', Offset(cx + r * 0.92, cy - r * 0.45), 13, Colors.white.withOpacity(0.70), center: true);
  }
