...
lossStreak: prevState.lossStreak,
    ...
    bullets.add('25%: 이동 ${movePct.toStringAsFixed(2)}% → 필요 레버리지 ${levNeedText}x');

    final roiOk = roiToTp >= 25.0;

    // 3) 최종 신호 표시
    // ✅ 구간 내부는 과매매 방지: 신호 비활성(관망)
    ...
    // edge(0~100): 중립(50)에서 얼마나 벗어났나. 20 이상이면 방향성 있다고 판단
    ...
    if (edge < MIN_EDGE) reasons.add('방향성 ${edge}%');
...
FuSignalLogger.append(out);
  ...
  Future<List<FuCandle>?> _tryBitgetCandles({required String symbol, required String tf}) async {
    // safeMode 에선 네트워크 호출을 줄여서 앱 안정성 우선
    ...
    for (final interval in intervals) {
      raw = await BitgetPublic.getCandlesRaw(
        ...
        symbol: symbol,
        interval: interval,
        // 캔들이 너무 적으면 구조/레벨/패턴이 안 보임.
        // 과도하게 크면 거래소가 거부할 수 있어 200으로 타협.
        limit: 200,
      );
  ...
  List<String>? _tfToBitgetIntervals(String tf) {
    ...
    switch (tl) {
      ...
      case '1w':
        return const ['1W'];
      default:
        // 원본이 1D/1W/1M 같은 대문자였다면 위에서 tl 로 커버됨
