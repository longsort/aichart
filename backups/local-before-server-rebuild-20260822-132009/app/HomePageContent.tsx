...
import { SYMBOLS, analysisMatchesSymbolAndTf, normalizeChartTimeframe } from '../lib/constants';
import {
...
import ChartView from './components/ChartViewLazy';
/** 통합·분석 · 독수리1호 공동 — 서버 ChartView 정적 로드(Lazy「준비 중」없음·캔들/전 TF) */
import ChartViewMergedServer from './components/ChartViewMergedServer';
import type { ChartSnapshotRef } from './components/chartSnapshotRef';
...
import Eagle1StructureDesk from './components/eagle1/Eagle1StructureDesk';
const AIChatPanel = dynamic(() => import('./components/AIChatPanel'), { ssr: false });
const AnalysisBoardHero = dynamic(() => import('./components/AnalysisBoardHero'), { ssr: false });
const MonthStartDeskCard = dynamic(() => import('./components/MonthStartDeskCard'), { ssr: false });
/** 기본 데스크·HUD는 정적 import — "올리는 중…"에 멈추지 않게 */
...
const UnifiedDeskDashboardGuide = dynamic(() => import('./components/UnifiedDeskDashboardGuide'), { ssr: false });
const AiAnalysisLineHints = dynamic(() => import('./components/AiAnalysisLineHints'), { ssr: false });
const FocusOverlay = dynamic(() => import('./components/FocusOverlay'), { ssr: false });
const ExecutionBriefingCard = dynamic(() => import('./components/ExecutionBriefingCard'), { ssr: false });
const FusionModeCard = dynamic(() => import('./components/FusionModeCard'), { ssr: false });
const AutonomousLearningCard = dynamic(() => import('./components/AutonomousLearningCard'), { ssr: false });
const CandleCompareCard = dynamic(() => import('./components/CandleCompareCard'), { ssr: false });
const SignalBox = dynamic(() => import('./components/SignalBox'), { ssr: false });
const VirtualTradeCard = dynamic(() => import('./components/VirtualTradeCard'), { ssr: false });
const TelegramMultiTfWatcher = dynamic(
  () => import('./components/TelegramMultiTfWatcher').then((m) => ({ default: m.TelegramMultiTfWatcher })),
  { ssr: false }
);
const TelegramMultiTfCard = dynamic(() => import('./components/TelegramMultiTfCard'), { ssr: false });
const TradeUnifiedGraph = dynamic(() => import('./components/TradeUnifiedGraph'), { ssr: false });
const TradePracticalDesk = dynamic(() => import('./components/TradePracticalDesk'), { ssr: false });
const TemporalComparePanel = dynamic(() => import('./components/TemporalComparePanel'), { ssr: false });
const TradeConfirmAlertBanner = dynamic(() => import('./components/TradeConfirmAlertBanner'), { ssr: false });
...
const ReferenceManager = dynamic(() => import('./components/ReferenceManager'), { ssr: false });
...
import { getStoredOpenAIKey, getStoredBriefingUser, getStoredBriefingPassword, addEstimatedCostUsd, peekSiteSessionHint, setSiteSessionHint, clearSiteSessionHint } from '@/lib/clientAiCredentials';
...
const ANALYZE_FETCH_RETRY_DELAY_MS = 280;
/** 통합·독수리 amx 분석은 30초+ 가능 — 조기 abort → 연결 오류 카드만 양산 */
const ANALYZE_CLIENT_TIMEOUT_MS = 90_000;
const ANALYZE_CLIENT_TIMEOUT_SHARED_MS = 110_000;
...
/** 상단 레일 — `localStorage` 저장. 최초(키 없음)는 메인(합성) — 통합·분석은 클릭 시에만. */
const RAIL_UI_MODE_STORAGE_KEY = 'ailongshort-rail-ui-mode-v2';
/** 1회: 잘못 메인에 묶인 통합·분석(Eagle1) 랜딩을 해제하고 메인(합성)으로 */
const MAIN_NOT_MERGED_LANDING_ONCE_KEY = 'ailongshort-main-not-merged-landing-v1';
function readStoredRailUiMode(): UIMode {
  if (typeof window === 'undefined') return 'UNIFIED_DESK';
  try {
    if (!window.localStorage.getItem(MAIN_NOT_MERGED_LANDING_ONCE_KEY)) {
      window.localStorage.setItem(MAIN_NOT_MERGED_LANDING_ONCE_KEY, '1');
      window.localStorage.setItem(RAIL_UI_MODE_STORAGE_KEY, 'UNIFIED_DESK');
      return 'UNIFIED_DESK';
    }
...
export default function HomePageContent() {
  ...
  useEffect(() => {
    const t = window.setTimeout(() => {
      hydrateFromServer().then((didHydrate) => {
        if (didHydrate) setVirtualTradeRefresh((n) => n + 1);
      });
    }, 800);
  ...
  /** 저장된 모드 즉시 복원 — EXECUTION 먼저 띄운 뒤 통합분석으로 갈아타면 차트가 두 번 올라감 */
  const [uiMode, setUiMode] = useState<UIMode>(readStoredRailUiMode);
  const [eagle1EvidenceOpen, setEagle1EvidenceOpen] = useState(false);
  ...
  /** 사이트 로그인 — 힌트가 있으면 접속확인 화면을 건너뛰고 바로 차트 시작 */
  const [siteAuth, setSiteAuth] = useState<'loading' | 'anon' | 'authed'>(() =>
    peekSiteSessionHint() ? 'authed' : 'anon'
  );
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    let settled = false;
    const finish = (next: 'anon' | 'authed', user = '') => {
      if (cancelled || settled) return;
      settled = true;
      setSiteAuth(next);
      if (user) setSiteUser(user);
      setAuthSlow(false);
    };
    const slowTimer = window.setTimeout(() => {
      if (!cancelled && !settled) setAuthSlow(true);
    }, 5000);
    fetch('/api/auth/session', { cache: 'no-store', credentials: 'same-origin', signal: ac.signal })
      ...
      .catch((e: { name?: string }) => {
        ...
        /** 컴파일·네트워크 실패 시 힌트 있으면 차트 유지 — 로그인 화면으로 튕기지 않음 */
        if (peekSiteSessionHint()) finish('authed');
        else finish('anon');
      });
    return () => {
      cancelled = true;
  ...
  const load = useCallback(async (overrideTf?: string, fastMode = false) => {
    ...
    const timeoutMs = uses4hShared ? ANALYZE_CLIENT_TIMEOUT_SHARED_MS : ANALYZE_CLIENT_TIMEOUT_MS;
    ...
    /** 공유모드·캐시 있으면 차트 먼저 — 분석은 백그라운드 (로딩 카드로 화면 막지 않음) */
    const softUi = uses4hShared && analysisUsableOnChart(analysisRef.current, symbol, chartTf, modeForAnalyze);
    if (!softUi) setLoading(true);
    ...
    try {
      ...
      if (controller.signal.aborted || e?.name === 'AbortError') {
        /** 새 요청으로 취소된 경우 무시. 타임아웃이어도 기존 분석·차트는 유지 */
        if (seq === analyzeSeqRef.current && analyzeAbortRef.current === controller) {
          const keep =
            analysisUsableOnChart(analysisRef.current, symbol, chartTf, modeForAnalyze) ||
            uses4hShared;
          if (!keep) {
            setError('분석이 시간 초과되었습니다. 새로고침하거나 잠시 후 다시 시도하세요.');
          } else {
            /** 차트·기존 분석 유지 — 연결 오류 카드로 화면 막지 않음 */
            setError(null);
          }
        }
        return;
      ...
      const msg = e?.message || '';
      if (uses4hShared && analysisUsableOnChart(analysisRef.current, symbol, chartTf, modeForAnalyze)) {
        setError(null);
        return;
      }
  ...
  const requestLoad = useCallback((overrideTf?: string) => {
    ...
    const sharedMode = isMergedDesk4hSharedAnalysisMode(modeRl);
    /** TF만 바꿀 때: 마켓 prefetch만 먼저 — 캔들 체감 최우선 */
    if (overrideTf) {
      prefetchMarketCandles(chartTf);
    }
    /**
     * 통합·분석·독수리1호: 이미 15m 분석이 있으면 TF 전환 시 /api/analyze 생략.
     * (캔들은 ChartView /api/market — 기능 삭제 아님)
     */
    if (sharedMode && overrideTf) {
      let hasUsable = false;
      for (const v of analysisCacheRef.current.values()) {
        if (analysisUsableOnChart(v, symbol, chartTf, modeRl)) {
          setAnalysis(decorateAnalysisWithSimilarReplay(v));
          hasUsable = true;
          break;
        }
      }
      if (!hasUsable && analysisUsableOnChart(analysisRef.current, symbol, chartTf, modeRl)) {
        hasUsable = true;
      }
      if (hasUsable) {
        setError(null);
      ...
      /** 15m 공동 분석이 아직 없으면 TF와 무관하게 즉시 로드 — 로딩 고착·데이터 없음 방지 */
      if (loadDebounceRef.current) clearTimeout(loadDebounceRef.current);
      loadDebounceRef.current = setTimeout(() => {
        loadDebounceRef.current = null;
        void load(chartTf, true);
      }, 0);
      return;
    }
    const analyzeTf = mergedDeskAnalyzeTimeframe(chartTf, modeRl);
    const pfK = parkfColorsCacheKey(
      parkfLinRegBaseHex,
      parkfLinRegLargeHex,
      parkfLinRegMediumHex,
    ...
    if (loadDebounceRef.current) clearTimeout(loadDebounceRef.current);
    /** TF 클릭: 지연 0 · 공유모드면 collect=0 fastMode로 즉시 표시 */
    const debounceMs = overrideTf != null ? 0 : 80;
    const useFast = sharedMode || overrideTf != null;
    loadDebounceRef.current = setTimeout(() => {
      loadDebounceRef.current = null;
      void load(chartTf, useFast);
    }, debounceMs);
  }, [
    load,
    symbol,
    timeframe,
    prefetchMarketCandles,
    decorateAnalysisWithSimilarReplay,
    zoneSignalSensitivity,
    pre3SimilarityThreshold,
    pre3ConfirmOnCloseOnly,
  ...
  useEffect(() => {
    if (siteAuth !== 'authed') return;
    prefetchMarketCandles(timeframeRef.current);
  ...
  }, [requestLoad, siteAuth, prefetchMarketCandles]);
  ...
  const pl = pageLayout;
  /** 독수리1호 Eagle1 HUD = 「독수리1호」 모드만 (통합·분석=서버 ARES) */
  ...
  const showRightStack =
    pl.showRightPanel &&
    (!useEagle1StructureDesk || eagle1EvidenceOpen) &&
    ...
    <>
      ...
      {pl.showGroupThemeAlerts && (
        <>
          ...
          {useEagle1StructureDesk && (
            <button
              type="button"
              className={`tool-chip tool-chip-button ${eagle1EvidenceOpen ? 'tool-chip-active' : ''}`}
              onClick={() => setEagle1EvidenceOpen((v) => !v)}
              title="기존 우측 분석 패널을 내부 Evidence로 열기"
            >
              내부 Evidence
            </button>
          )}
  ...
  if (siteAuth === 'loading') {
    return (
      <div style={{ minHeight: '50vh', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 8, color: 'var(--muted)' }}>
        ...
        {authSlow && (
          <>
            <span style={{ fontSize: 12, maxWidth: 360, textAlign: 'center', lineHeight: 1.45 }}>
              서버가 차트 코드를 컴파일 중이면 1–2분 걸릴 수 있습니다. 버튼을 반복해 누르지 말고 잠시 기다리세요.
            </span>
            <button
              type="button"
              onClick={() => {
    ...
    <>
      ...
      <main id="main-content" role="main">
        ...
        {confirmLastAlert && panelFeatures.practicalDesk && (
          <div style={{ position: 'sticky', top: 0, zIndex: 8400, padding: '0 0 10px' }}>
        ...
        {showHeaderCard && (
          <div className={`card header-card${!pl.showPageTitle ? ' header-card--toolbar-only' : ''}`}>
            {pl.showPageTitle && !useEagle1StructureDesk && (
              <div className="header-brand">
                <div className="ai-kicker">StructureAcceptanceEngine</div>
                <div className="title">독수리1호 AI 구조분석 시스템</div>
                <div className="subtle">APPROACH → TOUCH → WICK_BREAK → BODY_BREAK → CLOSE_CONFIRM → RETEST → HOLD → ACCEPTED</div>
            ...
            {anyMainToolbar && !pl.mainToolbarFloat && !useEagle1StructureDesk && (
              <div className="select-row">{mainToolbarBody}</div>
            )}
          </div>
        )}

        {layoutPortalsReady && pl.mainToolbarFloat && anyMainToolbar && !useEagle1StructureDesk &&
          createPortal(
            <div
        ...
        {mtfSignals.length > 0 && pl.showMtfStrip && !pl.mtfStripFloat && !useEagle1StructureDesk && (
        ...
        {layoutPortalsReady && mtfSignals.length > 0 && pl.showMtfStrip && pl.mtfStripFloat && !useEagle1StructureDesk &&
          createPortal(
            <div
        ...
        <div className={showRightStack ? 'grid' : 'grid grid--single'}>
          <div className="left-stack">
            <div className={`card panel-pad${uiMode === 'MERGED_ANALYSIS_DESK' ? ' card--merged-desk' : ''}${useEagle1StructureDesk ? ' card--eagle1-desk' : ''}`}>
              {pl.showChartCardHeader && uiMode !== 'MERGED_ANALYSIS_DESK' && !useEagle1StructureDesk && (
              ...
              /** 서버 ARES 통합·분석 — 서버 ChartView(캔들·전 TF) 정적 로드 */
              ...
              /** 독수리1호: 로컬 structureDesk ChartView 숨김 → 통합·분석과 동일 서버 차트 공동 사용 */
              <div className="chart-wrap chart-wrap--eagle1-structure chart-wrap--merged-host chart-wrap--merged-analysis">
                <MergedAnalysisDeskView
                  wrapEagle1Hud
          ...
          <div className="right-stack">
            ...
            <div className="card panel-pad" style={{ display: 'flex', flexDirection: 'column', minHeight: 380, maxHeight: 'min(86vh, 920px)', overflow: 'hidden' }}>
