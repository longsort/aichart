...
class TfConsensusRow {
  ...
  int get hit5 {
    ...
    if (r.evidence.risk <= 55) hit++; // risk는 낮을수록 좋음
    return hit;
  }

  String get dir {
    final t = r.decision.title;
    if (t.contains('숏') || t.toLowerCase().contains('short')) return 'SHORT';
    if (t.contains('롱') || t.toLowerCase().contains('long') || t.contains('상승')) return 'LONG';
    return 'NO';
  }
}
