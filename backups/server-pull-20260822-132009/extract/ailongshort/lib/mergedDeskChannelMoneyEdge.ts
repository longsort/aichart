...
function buildChannelTradeSetVisuals(
  ...
  const gateOn = waitingBreak || settled || failed;

  const entryTitle = failed
    ? `◆${viewKo}·실패금지·${fmt(plan.entry)}`
    : settled
      ? `◆${viewKo}·안착참고·${fmt(plan.entry)}`
      : plan.edgeStateKo === '돌파'
        ? `◆${viewKo}·안착대기·${fmt(plan.entry)}`
        : plan.edgeStateKo === '돌파가능'
          ? `◆${viewKo}·돌파전대기·${fmt(plan.entry)}`
          : `${sideTag}${viewKo}·E·${stance}`;
        ...
        : settled
          ...
          : waitingBreak
            ...
            : go
              ...
              : '#CA8A04',
      title: entryTitle,
      lineWidth: settled || waitingBreak ? 4 : 2,
      lineStyle: failed ? 'dotted' : 'solid',
      axisLabel: true,
    },
    {
      price: plan.stopLoss,
      color: '#F87171',
      title: gateOn ? `◆${viewKo}손절·${fmt(plan.stopLoss)}` : `손절SL·채널`,
      lineWidth: 2,
      lineStyle: 'dashed',
      axisLabel: true,
    },
  ];
  const tp1Color = isLong ? '#22C55E' : '#EF4444';
  const tp2Color = isLong ? '#3B82F6' : '#F472B6';
  const tp3Color = isLong ? '#2DD4BF' : '#FB7185';
  const tpTitle = (n: 1 | 2 | 3, ko?: string) => {
    const src = ko ? `·${ko}` : '';
    return gateOn ? `◆${viewKo}TP${n}${src}` : `익절TP${n}${src || '·채널'}`;
  };
  if (plan.tp1 > 0) {
    priceLines.push({
      price: plan.tp1,
      color: tp1Color,
      title: tpTitle(1, plan.tp1Ko),
      lineWidth: 2,
      lineStyle: 'dotted',
      axisLabel: true,
    });
