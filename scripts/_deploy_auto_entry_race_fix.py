...
def main():
    ...
    run(
        f"grep -n 'pickBtcSignalRaceWinner\\|canEnter: false\\|tickDualBgRaceEntries' "
        f"{remote}/lib/mergedDeskDualBgRaceEntry.ts "
        f"{remote}/app/components/mergedAnalysis/MergedAnalysisDeskView.tsx | head -20"
    )
    print("\nOK auto-entry race fix deploy", flush=True)
    ssh.close()
    return 0
