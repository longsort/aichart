...
class TfRolePanel extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    // EngineOutput ?¥Î? Íµ¨Ï°∞Í∞Ä ?ÑÎ°ú?ùÌä∏ÎßàÎã§ ?¨Îùº ?àÏ†Ñ?òÍ≤å "?ÜÏúºÎ©??®Í?"
    final eo = engineOutput;
    if (eo == null) return const SizedBox.shrink();

    // meta??tf alignment ?ïÎ≥¥Í∞Ä ?àÏúºÎ©??úÏö©
    ...
    map.forEach((k,v){
      ...
      if (s.toLowerCase().contains('ok') || s.toLowerCase().contains('agree') || s.contains('?©Ïùò')) c = Colors.greenAccent.withOpacity(0.8);
      if (s.toLowerCase().contains('conflict') || s.contains('Ï∂©Îèå')) c = Colors.redAccent.withOpacity(0.8);
