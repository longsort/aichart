...
function roleBase(role: ZoneRole, baseCaption: string, lang: MirageZoneFaceLang): string {
  ...
  if (lang === 'en') {
    if (first === '지지' || first === '저항' || first === '횡보') return ROLE_BASE_EN[role];
    if (/^OB/i.test(first) || first.includes('OB')) return ROLE_BASE_EN[role];
    return ROLE_BASE_EN[role];
  }
  if (first && ['지지', '저항', '횡보', '수요', '공급', '존'].some((k) => first.includes(k))) {
    return first.length <= 6 ? first : ROLE_BASE_KO[role];
  }
  return ROLE_BASE_KO[role];
}
...
export function pickMirageZoneFaceSignal(
  ...
  const skipBase = new Set(['지지', '저항', '횡보', '수요OB', '공급OB', '존']);
        ...
        : id.includes('support') || String(raw.overlayZoneExtraClass || '').includes('support')
          ...
          : id.includes('ob-bear')
            ...
            : id.includes('ob-bull')
              ? 'ob_bull'
              : 'neutral';
    const baseCaption = String(raw.label || '').split('·')[0]?.trim() || '존';
    const base = roleBase(role, baseCaption, lang);
