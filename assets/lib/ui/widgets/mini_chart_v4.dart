...
class MiniChartV4 extends StatefulWidget {
  ...
  /// 캔들 자체 분석 결과(EQL/EQH/BOS/CHOCH/MSB) — 전달 시 차트에 직접 작도
  final List<StructureMarkFu> structureMarks;

  /// 포커스 차트용: 균형선(VWAP), 강한고점/강한저점, 차트 위 떠있는 라벨(유동성·1W 마감·일 실행)
  final double vwap;
  final double? strongHigh;
  final double? strongLow;
  final List<String> chartFloatingLabels;
  ...
  const MiniChartV4({
    ...
    this.structureMarks = const [],
    this.vwap = 0,
    this.strongHigh,
    this.strongLow,
    this.chartFloatingLabels = const [],
    this.tfKey = '',
    this.nowAnchorKey,
  });
                              ...
                              child: RepaintBoundary(
                                child: SizedBox(
                                  ...
                                  child: CustomPaint(
                                    ...
                                    structureMarks: widget.structureMarks,
                                    vwap: widget.vwap,
                                    strongHigh: widget.strongHigh,
                                    strongLow: widget.strongLow,
                                    chartFloatingLabels: widget.chartFloatingLabels,
...
class _PV4 extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    // ===== 캔들 자체 분석 작도 (StructureMarksEngineFu 결과) =====
    if (structureMarks.isNotEmpty && (showBos || showChoch)) {
      void drawMarkTag(String txt, double x, double y0, Color col) {
        final tp = TextPainter(
          text: TextSpan(
            text: txt,
            style: TextStyle(color: col.withOpacity(labelOpacity), fontSize: 8, fontWeight: FontWeight.w800),
          ),
          textDirection: TextDirection.ltr,
        )..layout();
        final rr = RRect.fromRectAndRadius(
          Rect.fromLTWH(x - tp.width / 2 - 4, y0 - tp.height / 2 - 3, tp.width + 8, tp.height + 6),
          const Radius.circular(999),
        );
        canvas.drawRRect(rr, Paint()..color = col.withOpacity(0.2));
        canvas.drawRRect(rr, Paint()..color = col.withOpacity(0.4)..style = PaintingStyle.stroke..strokeWidth = 1);
        tp.paint(canvas, Offset(rr.left + (rr.width - tp.width) / 2, rr.top + (rr.height - tp.height) / 2));
      }
      for (final m in structureMarks) {
        if (m.index < 0 || m.index >= n) continue;
        final xx = xIdx(m.index);
        final yy = y(m.price);
        final col = m.tag == 'CHOCH'
            ? const Color(0xFFFF7A45)
            : m.tag == 'BOS' || m.tag == 'MSB'
                ? const Color(0xFF4DA3FF)
                : m.tag == 'EQH'
                    ? const Color(0xFF98A0C8)
                    : const Color(0xFF7C4DFF);
        drawMarkTag(m.tag, xx, yy, col);
      }
    }
    ...
    // 균형선(VWAP)
    if (vwap > 0 && vwap.isFinite) {
      final yy = y(vwap);
      final lineVwap = Paint()..color = t.accent.withOpacity(0.70)..strokeWidth = 1.6;
      for (double xx = plot.left; xx < plot.right; xx += 8) {
        canvas.drawLine(Offset(xx, yy), Offset(math.min(xx + 5, plot.right), yy), lineVwap);
      }
      levelLabel(vwap, '균형선', t.accent);
    }
    // 강한고점 / 강한저점
    if (strongHigh != null && strongHigh! > 0 && strongHigh!.isFinite) {
      final yy = y(strongHigh!);
      final lineH = Paint()..color = t.bad.withOpacity(0.55)..strokeWidth = 1.4;
      canvas.drawLine(Offset(plot.left, yy), Offset(plot.right, yy), lineH);
      levelLabel(strongHigh!, '강한고점', t.bad);
    }
    if (strongLow != null && strongLow! > 0 && strongLow!.isFinite) {
      final yy = y(strongLow!);
      final lineL = Paint()..color = t.good.withOpacity(0.55)..strokeWidth = 1.4;
      canvas.drawLine(Offset(plot.left, yy), Offset(plot.right, yy), lineL);
      levelLabel(strongLow!, '강한저점', t.good);
    }
    // 차트 위 떠있는 라벨(유동성 · 1W 마감 · 일 실행)
    if (chartFloatingLabels.isNotEmpty) {
      const double pad = 6;
      double top = plot.top + 4;
      for (final label in chartFloatingLabels) {
        if (label.isEmpty) continue;
        final tp = TextPainter(
          text: TextSpan(
            text: label,
            style: TextStyle(color: t.fg.withOpacity(0.92), fontWeight: FontWeight.w800, fontSize: 9),
          ),
          textDirection: TextDirection.ltr,
        )..layout();
        final rr = RRect.fromRectAndRadius(
          Rect.fromLTWH(plot.right - tp.width - pad * 2 - 4, top, tp.width + pad * 2, tp.height + pad),
          const Radius.circular(999),
        );
        canvas.drawRRect(rr, Paint()..color = t.card.withOpacity(0.85));
        canvas.drawRRect(rr, Paint()..color = t.border.withOpacity(0.4)..style = PaintingStyle.stroke..strokeWidth = 1);
        tp.paint(canvas, Offset(plot.right - tp.width - pad - 4, top + pad / 2));
        top += tp.height + pad + 4;
      }
    }
