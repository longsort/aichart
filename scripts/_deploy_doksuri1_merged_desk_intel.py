#!/usr/bin/env python3
"""Deploy Doksuri merged-desk intel briefing to /root/ailongshort."""
...
FILES = [
    "lib/mergedDeskCandleEventVerdict.ts",
    "lib/mergedAnalysisEntryMarkers.ts",
    "lib/mergedAnalysisDeskEngine.ts",
    "lib/doksuri1/focusSide.ts",
    "lib/doksuri1/coreBriefing.ts",
    "lib/doksuri1/mergedDeskIntel.ts",
...
def main() -> int:
    ...
    try:
        ...
        run(
            ssh,
            f"grep -n '핵심\\|$$$$분석\\|buildDoksuri1CoreLines\\|moneyAnalysisKo\\|가격대 불일치' "
            f"{REMOTE}/lib/doksuri1/coreBriefing.ts "
            f"{REMOTE}/lib/doksuri1/mergedDeskIntel.ts "
            f"{REMOTE}/lib/doksuri1/marketStory.ts "
            f"{REMOTE}/lib/doksuri1/telegramBridge.ts | head -n 40",
        )
        ...
        out("[done] doksuri merged-desk intel → /root/ailongshort")
    finally:
        sftp.close()
        ssh.close()
    return 0
