...
class AiOrderbookDetail extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final liqC = liq.contains('??)
        ? const Color(0xFFEA2A2A)
        : (liq.contains('??) ? const Color(0xFF1EEA6A) : const Color(0xFF4DA3FF));
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          _kv('?�동??, liq, c: liqC),
          const SizedBox(height: 4),
          _kv('?�프?�드', '${spreadBp}bp', c: spreadC),
          const SizedBox(height: 4),
          _kv('불균??, '${imbalanceP}%', c: cb),
