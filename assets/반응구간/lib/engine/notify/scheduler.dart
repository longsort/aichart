...
/// S-11: 매일 23:55 1일 마감 브리핑 자동 생성 후 알림. (앱 실행 중일 때 동작)
class Scheduler {
  ...
  Future<void> _runDailyBriefing() async {
    try {
      ...
      if (r is Err) {
        await _notify.notifyDailyBriefing('동기화 실패: ${(r as Err<String>).message}');
        return;
      }
      ...
      if (candles.isEmpty) {
        await _notify.notifyDailyBriefing('데이터 없음');
        return;
      }
      ...
      await _notify.notifyDailyBriefing('오류: ${e.toString().length > 50 ? e.toString().substring(0, 50) : e}');
    }
  }
}
