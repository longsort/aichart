  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: const Text('30일 백테스트(기록 기반)', style: TextStyle(fontWeight: FontWeight.w900)),
      ),
        ...
        builder: (c,s) {
          if (s.hasError) {
            return Center(child: Padding(
              ...
              child: Text('백테스트 실패: ${s.error}', style: const TextStyle(color: Colors.white70)),
            ));
          }
          ...
          return ListView(
            ...
            children: [
              _card('요약', Wrap(spacing: 10, runSpacing: 8, children: [
                _pill('트레이드 ${d.n}'),
                _pill('승률 ${d.winrate.toStringAsFixed(1)}%'),
                _pill('W/L ${d.w}/${d.l}'),
                _pill('기대값 ${d.avgR.toStringAsFixed(3)}R'),
                _pill('누적 ${d.sumR.toStringAsFixed(2)}R'),
                _pill('최대DD ${d.maxDD.toStringAsFixed(2)}R'),
              ])),
              const SizedBox(height: 10),
              _card('에퀴티(누적R)', SizedBox(
                height: 180,
...
class _EqPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    if (eq.isEmpty) {
      final tp = TextPainter(text: const TextSpan(text: '데이터 없음', style: TextStyle(color: Colors.white70)), textDirection: TextDirection.ltr);
      tp.layout();
