...
export function estimateBarBuySell(c: Candle): VolumeBarSplit {
  ...
  if ([hi, lo, cl, o].every(Number.isFinite) && hi > lo) {
    const range = hi - lo;
    const pos = clamp((cl - lo) / range, 0, 1);
    const upperWick = hi - Math.max(o, cl);
    const lowerWick = Math.min(o, cl) - lo;
    const upperR = upperWick / range;
    const lowerR = lowerWick / range;
    /** 종가 위치 기본 + 윗꼬리(고점 거부=매도)·아랫꼬리(저점 거부=매수) */
    let buyPct = clamp(0.18 + pos * 0.64, 0.15, 0.85);
    if (upperR >= 0.28 && pos <= 0.62) {
      /** 고점 긴 윗꼬리 — 매수가 많아 보여도 실제는 매도 소화(클리맥스) 흔적 */
      buyPct -= clamp(upperR * 0.55, 0.08, 0.32);
    }
    if (lowerR >= 0.28 && pos >= 0.38) {
      buyPct += clamp(lowerR * 0.55, 0.08, 0.32);
    }
    /** 음봉인데 종가가 레인지 상단이면 매수 과대평가 방지 */
    if (cl < o && pos > 0.55) buyPct -= 0.08;
    if (cl > o && pos < 0.45) buyPct += 0.06;
    buyPct = clamp(buyPct, 0.14, 0.86);
    const buyVol = vol * buyPct;
    const sellVol = vol - buyVol;
    const deltaPct = clamp((buyPct - 0.5) * 160, -70, 70);
    return finalizeSplit(buyVol, sellVol, buyPct, deltaPct, 'closePos');
  }

  /** 최후: 양/음봉만 — 전부 한색 금지, 60/40 소프트 분할 */
  const bull = cl >= o;
  const buyPct = bull ? 0.62 : 0.38;
  const buyVol = vol * buyPct;
  const sellVol = vol - buyVol;
  const deltaPct = bull ? 24 : -24;
