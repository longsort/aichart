...
import { collectBitgetSymbolPrice } from '@/lib/data/collectors/bitgetSymbolPriceCollector';
...
export async function fetchMarketData(symbol: string, timeframe: string): Promise<MarketDataResult> {
  const [
    ...
    withTimeout(
      collectBitgetSymbolPrice(symbol).catch(() => ({
        symbol: String(symbol || 'BTCUSDT').toUpperCase(),
        last: null,
        markPrice: null,
        indexPrice: null,
        ts: null,
        source: 'bitget-symbol-price' as const,
      })),
      4000,
      {
        symbol: String(symbol || 'BTCUSDT').toUpperCase(),
  ...
  const hasMark = bitgetSymbolPrice.markPrice != null && bitgetSymbolPrice.markPrice > 0;
  const hasIndex = bitgetSymbolPrice.indexPrice != null && bitgetSymbolPrice.indexPrice > 0;
  ...
  const moneyLive: Eagle1MoneyPressureLive = {
    ...
    ofi: ofiNow,
    has_ofi: hasOfi,
    markPrice: bitgetSymbolPrice.markPrice,
    indexPrice: bitgetSymbolPrice.indexPrice,
    has_mark: hasMark,
    has_index: hasIndex,
