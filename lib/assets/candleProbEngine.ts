/**
 * Fulink Pro ULTRA (assets/lib) CandleProbEngine 포팅.
 * 캔들 기반 조건부 확률·패턴·레짐 → 칩 목록 반환.
 */
import type { Candle } from '@/types';

export type ChipTone = 'good' | 'bad' | 'warn' | 'neutral';

export type ChipItem = { title: string; value: string; tone: ChipTone };
...
/** 조건부 표본: 현재 상태(장대/양·음/볼스파이크)와 같은 과거 인덱스 */
function conditionalSample(
  data: Candle[],
  now: { large: boolean; bull: boolean; volSpike: boolean }
): number[] {
  ...
  const isVolSpike = (c: Candle) => volAvg > 0 && (c.volume ?? 0) >= volAvg * 1.6;
  const indices: number[] = [];
  for (let i = 10; i < data.length - 6; i++) {
    const c = data[i];
    const bull = c.close >= c.open;
    if (now.large !== isLarge(c)) continue;
    if (now.volSpike !== isVolSpike(c)) continue;
...
function momentumProb(data: Candle[], horizon: number): number {
  ...
  const slopeVal = (recent[recent.length - 1].close - recent[0].close) / recent.length;
...
export function buildCandleProbChips(
  candles: Candle[],
  options: { currentDir?: string; currentProb?: number; sweepRisk?: number } = {}
): ChipItem[] {
  const minCandles = 12;
  ...
  const isVolSpike = volAvg > 0 && (last.volume ?? 0) >= volAvg * 1.6;

  const cond = { large: isLarge, bull: isBull, volSpike: isVolSpike };
  ...
  const dir = (options.currentDir || 'NEUTRAL').toUpperCase();
  const dirKo = dir === 'LONG' ? '롱' : dir === 'SHORT' ? '숏' : '관망';
  const sweepRisk = Math.min(100, Math.max(0, options.sweepRisk ?? 0));

  return [
    {
      title: '방향',
      value: options.currentProb && options.currentProb > 0 ? `${dirKo} ${options.currentProb}%` : dirKo,
      tone: dir === 'LONG' ? 'good' : dir === 'SHORT' ? 'bad' : 'neutral',
    },
    ...
    { title: '레짐', value: regime.label, tone: regime.tone },
    {
      ...
      tone: isLarge ? (isBull ? 'good' : 'bad') : 'neutral',
    },
    ...
    { title: '장대재출현', value: pctStr(replarge5), tone: toneByPct(replarge5) },
    {
      title: '스윕위험',
      value: `${sweepRisk}%`,
      tone: sweepRisk >= 70 ? 'bad' : sweepRisk >= 45 ? 'warn' : 'neutral',
    },
  ];
}
