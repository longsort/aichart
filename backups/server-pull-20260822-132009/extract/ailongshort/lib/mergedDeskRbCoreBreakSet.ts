...
export function buildMergedDeskRbCoreBreakSet(params: {
  ...
  if (showZones) {
    ...
    if (slippedCore) {
    ...
    } else if (phase === 'break' || phase === 'settle' || phase === 'confirm') {
      const ok = phase === 'confirm' || phase === 'settle';
      ...
      overlays.push(
        makeCoreZone({
          ...
          detailKo: [
            `${g.horizonKo || '채널'} ${dirKo} 핵심안착 zone`,
            ok
              ? side === 'below'
      ...
      overlays.push(
        makeCoreZone({
          ...
          faceBase: '핵심돌파',
          faceSignal: phase === 'pre' ? `${dirKo}대기` : `${dirKo}돌파`,
          detailKo: [
            `${g.horizonKo || '채널'} ${dirKo} 핵심 돌파면`,
            phase === 'pre' ? '종가가 레일을 넘기 전 — 대기' : '종가 이탈 · 형성봉 허그',
            '확정 수익·승률 보장 아님',
          ].join('\n'),
          t1: tZone1,
          t2: tZone2,
