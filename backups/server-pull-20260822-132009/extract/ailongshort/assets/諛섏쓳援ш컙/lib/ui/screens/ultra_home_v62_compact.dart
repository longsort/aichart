...
import 'package:fulink_pro_ultra/ui/widgets/v62_visual_pack.dart';
import 'package:fulink_pro_ultra/engine/core/core_engine.dart';
import 'package:fulink_pro_ultra/engine/core/core_demo_feed.dart';
          ...
          children: [
            ...
            if (!compact)
              Expanded(
                child: ListView(
                  children: [
                    _glassCard(child: _section('Key Levels', '다음 단계: 지지/저항 자동 집계 연결')),
                    _glassCard(child: _section('Evidence', '다음 단계: evidenceHit(10종) 연결')),
                    _glassCard(child: _section('Scenarios', '다음 단계: Entry/SL/TP 자동 산출')),
                  ],
                ),
              )
            else
      ...
      body: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            const Text('※ 현재는 CORE 데모 피드 기반.\n다음 단계: 실시간 거래소 데이터 연결 + 통계/학습 저장.', style: TextStyle(fontSize: 13, color: Colors.white70)),
          ],
        ),
      ),
    );
