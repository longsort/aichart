...
void _startFlow() {
  // 네 프로젝트 구조(리스크관리까지.zip) 기준: BitgetApi 호출 메서드/경로가 환경별로 달라 빌드가 깨질 수 있음.
  // v1은 "컴파일 안정" 우선: 캔들/반응구간 기반으로 FlowSnapshot을 추정값으로 채움.
  _poll?.cancel();
  _poll = Timer.periodic(const Duration(seconds: 2), (_) {
    try {
      ...
      // tape: 최근 1캔들 방향 기반(간단 휴리스틱)
      final tape = (50 + (delta == 0 ? 0 : (delta > 0 ? 18 : -18))).clamp(0, 100).toInt();

      // ob: 반응구간 중앙 근처면 '방어 잔량' 있다고 가정(중립 보정)
      ...
      // absorption: 반응구간 근접 + 되돌림(스파이크) 가정
      final absorb = (50 + (near * 18.0)).round().clamp(0, 100);

      // cvd: 부호만 간단히
      final cvd = delta == 0 ? 0.0 : (delta > 0 ? 1.0 : -1.0);
      ...
      // Bias: mtfPulse 있으면 다수결, 없으면 위치 기반
      bool isLongBias;
