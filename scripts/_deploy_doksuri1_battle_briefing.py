#!/usr/bin/env python3
"""Deploy Doksuri-1 battle briefing to /root/ailongshort."""
...
def main() -> int:
    ...
    try:
        ...
        run(ssh, f"mkdir -p '{REMOTE}/app/components/mergedAnalysis'")
        for rel in FILES:
        ...
        run(
            ssh,
            f"grep -n 'buildDoksuri1Pack\\|Doksuri1BattleCard\\|chartMergedDeskDoksuri1BriefingEnabled' "
            f"{REMOTE}/lib/telegramMergedDeskAutoRunner.ts "
            f"{REMOTE}/lib/settings.ts "
            f"{REMOTE}/app/components/mergedAnalysis/MergedAnalysisDeskView.tsx | head -n 30",
        )
        ...
        out("[done] doksuri1 battle briefing deployed")
    finally:
        sftp.close()
        ssh.close()
    return 0
