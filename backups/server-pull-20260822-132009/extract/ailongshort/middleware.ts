...
export async function middleware(request: NextRequest) {
  ...
  if (process.env.NODE_ENV === 'production' && (!process.env.APP_SESSION_SECRET?.trim() || secret === 'ailongshort-dev-session-secret')) {
    return NextResponse.json(
      { error: 'APP_SESSION_SECRET 미설정 — 운영에서 긴 임의 문자열을 넣으세요.', code: 'SITE_AUTH_MISCONFIGURED' },
      { status: 503 }
    );
  }
