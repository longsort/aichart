...
const HUB_BADGE_CLASS: Record<'long' | 'short' | 'neutral' | 'warn' | 'info', string | undefined> = {
  long: styles.integratedSubBadge_long,
  short: styles.integratedSubBadge_short,
  neutral: styles.integratedSubBadge_neutral,
  warn: styles.integratedSubBadge_warn,
  info: styles.integratedSubBadge_info,
};
...
const HubSection = forwardRef<HTMLElement, HubSectionProps>(function HubSection(
  { title, subtitle, badge, badgeTone = 'info', children, defaultOpen = false },
  ref,
) {
  const storageKey = `ailongshort-fold-card:hub-${title}`;
  ...
  const badgeClass = HUB_BADGE_CLASS[badgeTone] ?? styles.integratedSubBadge_info;
  ...
  return (
    <section ref={ref} className={styles.integratedSection}>
      ...
      >
        ...
        {badge ? (
          <span className={`${styles.integratedSectionBadge} ${badgeClass}`}>
            {badge}
          </span>
        ) : null}
        <span className={styles.integratedSectionToggle}>{open ? '접기' : '펴기'}</span>
      </button>
