...
function buildOneParallelChannel(params: {
  ...
  const tDrawEnd = mergedDeskRbFutureTime2(candles, tEnd, iEnd);
  ...
  const iFut = iEnd + MERGED_DESK_RB_FUTURE_BARS;
  const up2 = upAt(iFut);
  const lo2 = loAt(iFut);
...
function buildConfluenceCorridor(
  ...
  const tDrawEnd = mergedDeskRbFutureTime2(candles, tLast, iEnd);

  /** 단기 기울기(봉당)로 tip 중착 폭을 뒤로 투영 · 우측 10봉 예측 */
  const bars = Math.max(1, iEnd - iStart);
  const slopePerBar = (shortG.tipUpper - shortG.up1) / bars;
  const up2 = tipHi + slopePerBar * MERGED_DESK_RB_FUTURE_BARS;
  const lo2 = tipLo + slopePerBar * MERGED_DESK_RB_FUTURE_BARS;
