...
import { applyMoneyPressureToRbOverlays } from '@/lib/eagle1/moneyPressureBand';
...
export default function MergedAnalysisDeskView({
  ...
  const syncFullscreen = useCallback(() => {
    ...
    setChartFullscreen(
      document.fullscreenElement === chartColRef.current ||
        !!chartColRef.current?.classList.contains('is-css-chart-fullscreen')
    );
  }, []);
  ...
  /** React 상태와 CSS FS 클래스 동기 — 깜빡임 없이 유지 */
  useEffect(() => {
    const el = chartColRef.current;
    if (!el || isMobileLikeViewport()) return;
  ...
  useEffect(() => {
    ...
    const onKey = (e: KeyboardEvent) => {
      ...
      const el = chartColRef.current;
      el?.classList.remove('is-css-chart-fullscreen');
      document.documentElement.classList.remove('eagle1-chart-fs-active');
  ...
  const toggleChartFullscreen = async () => {
    ...
    /** 네이티브 FS는 트리 리마운트와 충돌해 깜빡임만 남김 — CSS 고정 FS만 사용 */
    if (document.fullscreenElement) {
      try {
  ...
  const [chartMountReady] = useState(true);
  ...
  const deskPackForChart = useMemo(() => {
    ...
    const finalizeCoreMoneyZones = (list: OverlayItem[]) => {
      ...
      if (deskCandles.length >= 8) {
        const src = deferredAnalysisReady ? deferredAnalysis : analysis;
        next = applyMoneyPressureToRbOverlays(next, deskCandles, {
          buyPressure: src?.buyPressure,
          sellPressure: src?.sellPressure,
          volumeDelta: src?.volumeDelta,
          orderbookImbalance: src?.orderbookImbalance,
          has_cvd: src?.eagle1Availability?.has_cvd,
          has_orderbook: src?.eagle1Availability?.has_orderbook,
          has_trades: src?.eagle1Availability?.has_trades,
        });
      }
      return dedupe(next);
    };
  ...
  const selectedMirageOverlay = useMemo(() => {
    if (!selectedMirageZoneId) return null;
    const fromDesk =
    ...
    const src = deferredAnalysisReady ? deferredAnalysis : analysis;
    if (!selectedMirageZoneId.startsWith('eagle1-') || !src?.eagle1Zones) return null;
    const last = deskCandles[deskCandles.length - 1];
    const lastTime = Number(last?.time) > 0 ? Number(last.time) : Date.now();
    return overlayFromEagle1ZoneId({
      id: selectedMirageZoneId,
      lastTime,
      mode: src.eagle1ChartUx?.mode ?? 'practical',
      pack: {
        zones: src.eagle1Zones.zones,
        clusters: src.eagle1Zones.clusters,
        recommended: src.eagle1Zones.recommended,
        displaySupport: src.eagle1Zones.displaySupport,
        displayResist: src.eagle1Zones.displayResist,
        pocState: src.eagle1Zones.profile?.pocState,
        reaction: src.eagle1Zones.reaction,
      },
    });
  }, [
    selectedMirageZoneId,
  ...
  const handleMirageZoneSelect = useCallback(
    (zoneId: string) => {
      ...
      const lastTime = Number(last?.time) > 0 ? Number(last.time) : Date.now();
      const overlay =
        deskPackForChart?.overlays?.find((o) => String(o.id) === zoneId) ??
        (zoneId.startsWith('eagle1-') && src?.eagle1Zones
          ? overlayFromEagle1ZoneId({
              id: zoneId,
              lastTime,
              mode: src.eagle1ChartUx?.mode ?? 'practical',
              pack: {
  ...
  const shell = (
    <div
      ...
      data-eagle1-chart-host="merged"
    >
      {!fsActive && (
        <header className={styles.mergedDeskHeader} hidden={wrapEagle1Hud || undefined}>
          <div>
            <div className={styles.mergedDeskTitle}>
              {wrapEagle1Hud ? '독수리1호 AI 구조분석 시스템' : 'ARES · 스윙 차트'}
            </div>
            <div className={styles.mergedDeskSub}>
              {wrapEagle1Hud
                ? `StructureAcceptanceEngine · ${symbol} · ${timeframe} · ${candleSourceKo}`
      ...
      <div className={`${styles.mergedDeskBody}${fsActive ? '' : ` ${styles.mergedDeskChartFirst}`}`}>
        <div
          ref={chartColRef}
        ...
        >
          {wrapEagle1Hud ? (
          <div className={styles.eagle1ModeFsStrip} data-eagle1-mode-fs="1">
            <UIModeSwitcher
              uiMode={uiMode}
              setUiMode={onUiModeChange}
              compact
              className={styles.structureModeSwitch}
            />
            <button
              type="button"
              className={`${styles.eagle1FsChip}${fsActive ? ` ${styles.eagle1FsChipOn}` : ''}`}
              data-eagle1-fs-chip="strip"
              onClick={() => void toggleChartFullscreen()}
              title={fsActive ? '전체화면 종료 (Esc)' : '차트 전체화면'}
            >
              {fsActive ? '⛶ 종료' : '⛶ 전체화면'}
            </button>
          </div>
          ...
          <div
            ...
            {...(wrapEagle1Hud ? { 'data-eagle1-desk-toolbar': '1' } : { 'data-merged-ares-toolbar': '1' })}
          >
            <div className={styles.tfToolbar} aria-label="타임프레임">
              ...
              {wrapEagle1Hud && !fsActive && (
                <button
                  type="button"
                  className={`tool-chip tool-chip-button ${structureToolsOpen ? 'tool-chip-active' : ''}`}
                  onClick={() => setStructureToolsOpen((v) => !v)}
                  title="기존 통합·분석 도구 — 기능 유지, 차트 위에 펼침"
                >
                  {structureToolsOpen ? '도구닫기' : '도구'}
                </button>
              )}
              {wrapEagle1Hud && !fsActive && (
                <UIModeSwitcher
            ...
            <div
              className={`${styles.mergedToolbarActions}${
                wrapEagle1Hud && !fsActive && !structureToolsOpen ? ` ${styles.mergedToolbarActionsCollapsed}` : ''
              }`}
            >
              {mobileFsActive && (
                <button
