...
/** Strike·분석 병합 후 TP 사다리 정렬·빈 칸 RR 보강 */
export function strengthenUnifiedDeskTradePlan(plan: UnifiedDeskTradePlan): UnifiedDeskTradePlan {
  if (plan.direction === 'NEUTRAL' || plan.entry <= 0 || plan.stopLoss <= 0) return plan;

  const { direction, entry, stopLoss } = plan;
  const risk = Math.abs(entry - stopLoss);
