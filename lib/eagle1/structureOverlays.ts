...
export function eagle1StructureToOverlays(params: {
  ...
  /** 연속 2회만 봉 핀 추가. 기존 BOS/FAIL/존은 그대로. */
  for (const ev of pickVisibleSweepEvents(live, 80, 2)) {
    const id = `eagle1-sweep-pin-${ev.known_at}-${ev.index}`;
    if (seen.has(id)) continue;
    seen.add(id);
    const t1 = Number(candles[ev.known_at]?.time ?? candles[ev.index]?.time ?? lastTime);
    const px = Number(ev.price) || Number(ev.level);
    if (!(t1 > 0) || !(px > 0)) continue;
    out.push({
      id,
      kind: 'label',
      label: 'SWEEP',
      ...
      structureBias: ev.bias,
      noProject: true,
      overlayZoneExtraClass: 'eagle1-structure eagle1-sweep-candle-pin',
      labelTooltip: ev.evidence.join(' · ') || 'SWEEP',
    });
  }
  return out;
}
