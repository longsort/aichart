...
function formatBandTouchChartText(ev: InstitutionalBandInteractionMarker): string {
  return formatInstitutionalBandTouchMarkerChartText(ev);
}

function bandTouchMarkerStyle(ev: InstitutionalBandInteractionMarker): { color: string; size: number } {
  return institutionalBandTouchMarkerStyle(ev);
}
...
/**
 * 선반영 관찰/준비 원(파랑·노랑) — 차트에 올리면 존·선·클릭을 가려서 기본 비표시.
 * 시그니처·호출부는 유지(삭제 금지). 엔진 상태 자체는 그대로 동작.
 */
export const MERGED_DESK_FRONT_RUN_CHART_MARKERS_ENABLED = false;
...
export function buildMergedDeskFrontRunMarker(
  ...
  if (!MERGED_DESK_FRONT_RUN_CHART_MARKERS_ENABLED) return null;
  if (frontRun.state === 'NO_SIGNAL' || frontRun.state === 'INVALID') return null;
  /** 관찰(WATCH) 원·라벨은 항상 숨김 — 다른 작도/클릭을 막음 */
  if (frontRun.state === 'WATCH') return null;
  const dir =
  ...
  const frStateKo = frontRun.state === 'READY' ? '준비' : '신호없음';
  const frDirKo = dir === 'LONG' ? '롱' : dir === 'SHORT' ? '숏' : '';
  const frColor = frontRun.state === 'READY' ? '#F59E0B' : '#94A3B8';
...
export function consolidateMergedDeskPulseMarkers(params: {
  ...
  /** 파랑「선반영 관찰 롱/숏」등 — 차트 클릭·존을 가리므로 제거 */
  const withoutWatchFr = raw.filter((m) => {
    const tx = String(m.text || '');
    if (/선반영\s*관찰/.test(tx)) return false;
    if (/^관찰\s*(롱|숏)/.test(tx)) return false;
    if (String(m.id || '').includes('frontrun') && /관찰/.test(tx)) return false;
    return true;
  });

  const byTime = new Map<number, UnifiedPulseChartMarker[]>();
  for (const m of withoutWatchFr) {
    const tm = Number(m.time);
...
/** 다른 모드와 동일 — ST 터치. zone 모드면 화살/숏ST 글자 마커 생략(면으로만). */
export function buildMergedDeskInstitutionalBandMarkers(params: {
  ...
  if (MERGED_DESK_ST_TOUCH_AS_ZONES) return [];
