...
export function buildMasterFuturesDecision(params: {
  ...
  const entryRaw = tradePlan.entry > 0 ? tradePlan.entry : 0;
  const stopRaw = tradePlan.stopLoss > 0 ? tradePlan.stopLoss : 0;
  const tp1Raw = tradePlan.tp1 > 0 ? tradePlan.tp1 : 0;
  const geom =
    (planDir === 'LONG' || planDir === 'SHORT') && entryRaw > 0
      ...
      : null;
  const entryPrice = geom?.entry ?? entryRaw;
  const stopPrice = geom?.stopLoss ?? stopRaw;
  const tp1 = geom?.tp1 ?? tp1Raw;
  const rr =
