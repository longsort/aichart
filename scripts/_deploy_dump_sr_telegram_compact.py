#!/usr/bin/env python3
"""Deploy dump SR + telegram compact briefing to /root/ailongshort only."""
...
FILES = [
    # telegram compact + E/SL/TP emoji
    ...
    # dump SR score + viewmodel
    "lib/mergedDeskSupportResistScore.ts",
...
def main() -> int:
    ...
    try:
        ...
        run(
            ssh,
            f"grep -n 'buildTelegramCoreConclusionKo\\|resolveTelegramGuideLevels\\|지지조건부\\|fef9c3' "
            f"{REMOTE}/lib/telegramAlertBriefing.ts "
            f"{REMOTE}/lib/mergedDeskSupportResistScore.ts "
            f"{REMOTE}/lib/mergedDeskDumpLifeCycle.ts | head -n 40",
        )
        ...
        run(ssh, "pm2 ls | head -n 30", timeout=60)
        out("[done] dump-SR + telegram compact deployed to /root/ailongshort")
    finally:
        sftp.close()
        ssh.close()
    return 0
