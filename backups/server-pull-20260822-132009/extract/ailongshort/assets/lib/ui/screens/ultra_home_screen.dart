...
import '../widgets/focus_liquidity_strip.dart';
      ...
      child: Column(
        ...
        children: [
          ...
          Container(
            ...
            child: Builder(
              builder: (context) {
                final lux = AdaptiveLuxTrendline.compute(candles: _s.candles, tfKey: tf);
                final luxOv = luxTlToOverlay(r: lux, candles: _s.candles);
                final hasPlan = _s.entry > 0 && _s.stop > 0 && _s.target > 0;
                final marks = StructureMarksEngineFu.build(_s.candles, maxMarks: 16);
                double? strongHigh;
                double? strongLow;
                ...
                if (strongHigh == null && _s.candles.length >= 10) {
                  final recent = _s.candles.sublist(_s.candles.length - 30);
                  strongHigh = recent.map((c) => c.high).reduce((a, b) => a > b ? a : b);
                }
                if (strongLow == null && _s.candles.length >= 10) {
                  final recent = _s.candles.sublist(_s.candles.length - 30);
                  strongLow = recent.map((c) => c.low).reduce((a, b) => a < b ? a : b);
                }
                final ob = _s.obImbalance.clamp(0, 100);
                final liquidityStr = ob == 50 ? '유동성 균형' : (ob > 50 ? '유동성 매수우세' : '유동성 매도우세');
                final w1 = CandleCloseUtil.evaluate(tfLabel: '1W', price: livePrice, vwap: _s.vwap, score: _s.score, confidence: _s.confidence, risk: _s.risk);
                final d1 = CandleCloseUtil.evaluate(tfLabel: '1D', price: livePrice, vwap: _s.vwap, score: _s.score, confidence: _s.confidence, risk: _s.risk);
                final chartFloatingLabels = [
                  liquidityStr,
                  '1W ${w1.verdict} ${CandleCloseUtil.fmtRemain(w1.remaining)}',
                  '일실행 1D ${d1.verdict} ${CandleCloseUtil.fmtRemain(d1.remaining)}',
                ];
                return MiniChartV4(
                  candles: _s.candles,
                  obZones: _s.obZones,
                  ...
                  showBOS: AppSettings.I.showBOS.value,
                  showCHoCH: AppSettings.I.showCHoCH.value,
                  structureMarks: marks,
                  vwap: _s.vwap,
                  strongHigh: strongHigh,
                  strongLow: strongLow,
                  chartFloatingLabels: chartFloatingLabels,
                  tfKey: tf,
                  bias: _s.decisionTitle.contains('롱') ? 'LONG' : _s.decisionTitle.contains('숏') ? 'SHORT' : null,
                  prob: _s.confidence,
                  showPlan: hasPlan,
                  entry: _s.entry,
                  stop: _s.stop,
                ...
                Navigator.of(context).push(
                  MaterialPageRoute(
                    builder: (_) => ChartFullScreenPage(
                      ...
                      r1: _s.r1,
                      structureTag: _s.structureTag,
                      structureMarks: StructureMarksEngineFu.build(_s.candles, maxMarks: 12),
                      bias: _s.decisionTitle.contains('롱') ? 'LONG' : _s.decisionTitle.contains('숏') ? 'SHORT' : null,
                      prob: _s.confidence,
                      showPlan: _s.entry > 0 && _s.stop > 0 && _s.target > 0,
                      entry: _s.entry,
                      stop: _s.stop,
                      target: _s.target,
                    ),
          ...
          const SizedBox(height: 8),
          FocusLiquidityStrip(s: _s, livePrice: livePrice, t: t),
          const SizedBox(height: 8),
          CsvChipRowV1(
            t: t,
            candles: _s.candles,
