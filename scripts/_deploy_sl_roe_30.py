...
def main():
    ...
    # force arm SL 30 on server
    run(
        "python3 - <<'PY'\n"
        "import json,glob\n"
        "for p in glob.glob('/root/ailongshort/data/auto-trade-arm/*.json'):\n"
        "  if 'fired' in p or 'exit-phase' in p: continue\n"
        "  try:\n"
        "    d=json.load(open(p))\n"
        "    d['scalpSlRoePct']=30\n"
        "    json.dump(d, open(p,'w'), ensure_ascii=False, indent=2)\n"
        "    print('patched', p, 'sl=30')\n"
    ...
    run(
        f"grep -n 'FAST_SL_ROE_PCT = 30\\|손절≥\\|scalpSlRoePct' "
        f"{remote}/lib/mergedDeskAutoTradeConfig.ts "
        f"{remote}/lib/serverMergedDeskAutoTradeStore.ts | head -20"
    )
    print("\nOK sl-roe-30 deploy", flush=True)
    ssh.close()
    return 0
