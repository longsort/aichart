...
/** TF별 반등 학습 적중률 — dump rebound confluence 가중치용 */
export function summarizeBounceLearnStats(timeframe?: string): {
  hitRate: number | null;
  sample: number;
  hits: number;
} {
  const log = loadBounceLearnLog();
  const scoped = timeframe
    ? log.filter((e) => String(e.timeframe || '') === timeframe)
    : log;
  const recent = scoped.slice(0, 24);
  if (recent.length < 4) {
    return { hitRate: null, sample: recent.length, hits: recent.filter((e) => e.hit).length };
  }
  const hits = recent.filter((e) => e.hit).length;
  return { hitRate: hits / recent.length, sample: recent.length, hits };
}
