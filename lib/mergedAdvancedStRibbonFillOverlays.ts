...
export function buildMergedAdvancedStRibbonFillOverlays(params: {
  ...
  paintSegs.forEach((seg, idx) => {
    ...
    out.push({
      ...
      overlayZoneExtraClass: 'merged-ares-st-cloud merged-desk-st-cloud-keep merged-desk-zone-label-on',
      zoneFaceBase: longSeg ? '롱기관밴드' : '숏기관밴드',
      labelTooltip: '기관밴드 강화(마감·고급 ST 구름) · 상·하한 사이 채움 · 참고·비보장',
    });
  });
