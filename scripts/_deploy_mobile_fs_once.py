#!/usr/bin/env python3
"""Upload post-deploy label/zone/rocket set → /root/ailongshort → build + pm2."""
...
FILES = [
    ...
    "lib/mergedDeskBlueRedLabelGuide.ts",
    ...
    "app/components/AppSiteLogin.tsx",
]
...
def main() -> int:
    ...
    try:
        ...
        run(
            ssh,
            "cd /root/ailongshort && pm2 restart ailongshort && sleep 3 && "
            "curl -sf -o /dev/null -w 'home=%{http_code}\\n' http://127.0.0.1:3000/ && "
            "pm2 list | sed -n '1,8p'",
        )
        out("[done] all-tf zoom-attach label deploy")
    finally:
        sftp.close()
        ssh.close()
    return 0
