...
function seriesFile(symbol: string, root: string): string {
  return path.join(root, 'data', 'eagle1', 'series', seriesName(symbol));
}
...
export function loadMicrostructureSeries(symbol: string, root = process.cwd()): MicrostructureFile {
  try {
    const raw = JSON.parse(fs.readFileSync(seriesFile(symbol, root), 'utf8')) as MicrostructureFile;
...
export function ingestMicrostructureSeries(
  ...
  fs.mkdirSync(path.join(root, 'data', 'eagle1', 'series'), { recursive: true });
  fs.writeFileSync(seriesFile(next.symbol, root), JSON.stringify(next), 'utf8');
  return next;
}
