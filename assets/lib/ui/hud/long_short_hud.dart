...
class LongShortHud extends StatelessWidget{
  ...
  Widget build(BuildContext context){
    return Column(children:[
      Text('상승 ${(longP*100).toStringAsFixed(1)}%', style: TextStyle(color:Colors.greenAccent)),
      LinearProgressIndicator(value: longP, valueColor: AlwaysStoppedAnimation(Colors.greenAccent)),
      const SizedBox(height:6),
      Text('하락 ${(shortP*100).toStringAsFixed(1)}%', style: TextStyle(color:Colors.redAccent)),
