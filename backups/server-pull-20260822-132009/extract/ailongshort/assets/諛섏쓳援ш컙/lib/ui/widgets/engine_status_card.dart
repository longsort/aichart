      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            Row(
              children: [
                const Text('엔진 상태', style: TextStyle(fontSize: 13, fontWeight: FontWeight.w800)),
                const Spacer(),
                Text('WS ${_age(lastWs)} · RELOAD ${_age(lastReload)}', style: TextStyle(fontSize: 11, color: Colors.white.withOpacity(0.65))),
              ],
            ),
            const SizedBox(height: 8),
            _row(label: '확률 엔진', on: probOn, tap: onToggleProb),
            _row(label: '이벤트 영향', on: eventOn, tap: onToggleEvent),
            _row(label: '체결 강도', on: tapeOn, tap: onToggleTape),
            Text('※ 클릭하면 ON/OFF', style: TextStyle(fontSize: 10, color: Colors.white.withOpacity(0.55))),
          ],
        ),
      ),
    );
