...
export function formatTelegramTradeConfirmHtml(params: {
  ...
  lines.push(`<b>${emoji} ${escapeTelegramHtml(desk.notifyTitle.replace(/^[✓●★◐⚠]\s*/, ''))}</b>`);
  lines.push(`<b>${escapeTelegramHtml(symbol)}</b> · ${escapeTelegramHtml(timeframe)}${uiModeKo ? ` · ${escapeTelegramHtml(uiModeKo)}` : ''}`);
  lines.push('');
  ...
  if (desk.invalidPrice != null) {
    ...
    lines.push(`<b>무효가</b> ${escapeTelegramHtml(inv)}`);
  }
  if (desk.tp1 != null) {
    const tp =
      desk.tp1 >= 1000 ? desk.tp1.toLocaleString(undefined, { maximumFractionDigits: 2 }) : desk.tp1.toFixed(2);
    lines.push(`<b>TP1</b> ${escapeTelegramHtml(tp)}`);
  }
  lines.push(`<b>확정</b> ${desk.gatesPassCount}/5${desk.isFullConfirm ? ' · MTF통과' : ''}${desk.mtfBlocked ? ' · MTF보류' : ''}`);
  lines.push('');
